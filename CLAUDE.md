...
Behavioural tests are **SystemTests**: each one drives the running world from a high-level **macro** (a generator that asks the live world where things are and synthesises real input events), then compares canvas screenshots pixel-by-pixel against reference images. The Automator source and 196 tests live in the sibling `Fizzygum-tests` repo (not here); any non-`--homepage` build copies them in. (Macros are the only authoring path — the old input *recorder* has been removed.)
...
- **Every widget is PLACED and SIZED in integer pixels** (bounds rounded at `Widget._moveToNoSettle`/`__commitExtent`; fractional *desired* geometry is kept on the side, e.g. `positionFractionalInHoldingPanel` for stretchable panels). Internal *content* rendering (vector icons, charts, rotated strokes) is legitimately fractional. Integer placement is **necessary but not sufficient** for a back buffer to be byte-identical to a direct draw. Policy + rationale + the back-buffer FP caveat: [`docs/integer-pixel-placement-and-sizing.md`](docs/integer-pixel-placement-and-sizing.md).
