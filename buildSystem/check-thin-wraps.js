...
// MECHANICAL THIN WRAP around its private *Core twin. Mirrors check-dead-methods.js / check-stinks.js
// (line scanner; exit 0 clean / 1 violation).
//
// CANONICAL FORM: for a private `_<name>Core`, the public `<name>` in the SAME class must, after
// comments/blank lines, be:
//     [ zero or more  `return if <cond>` / `return unless <cond>` idempotency guards ]
//     @mutateGeometryThenSettle => @_<name>Core <args?>
...
//
// One class per file (filename == class name). A `_<name>Core` with no same-class `<name>` (e.g.
// BasementWdgt._addLostWidgetCore, reached only internally) is skipped -- there is no public twin to
...
for (const p of walk(SRC, [])) {
  ...
  for (let k = 0; k < heads.length; k++) {
    const m = /^_(.+)Core$/.exec(heads[k].name);
    if (!m) continue;                                   // not a *Core method
    const base = m[1];                                  // _fullDestroyCore -> fullDestroy
    ...
    const canonical = new RegExp('^@mutateGeometryThenSettle\\s*=>\\s*@_' + base + 'Core\\b');
...
if (violations.length) {
  console.error(`\n[thin-wraps] FAIL -- ${violations.length} public method(s) own a _<name>Core twin but are neither the canonical wrap nor marked exempt:`);
  for (const v of violations) {
    console.error(`  ${v.key}  (${v.file}:${v.line})`);
    console.error(`    body: ${JSON.stringify(v.body)}`);
  }
  console.error('\nCanonical: [return if/unless guards] then  @mutateGeometryThenSettle => @_<name>Core <args>.');
  console.error('Or add a  # thin-wrap-exempt: <reason>  comment directly above the method (see the 3 in WorldWdgt).');
  process.exit(1);
}
console.log('[thin-wraps] OK -- every public/Core pair is the canonical wrap or marked exempt.');
process.exit(0);
