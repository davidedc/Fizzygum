...
const STINKS = [
  ...
  { id: 'instanceof-type-test', baseline: 87,   // Pharo: ReBadMessageRule (isKindOf:); tightened 105->97 (2026-07-17); 97->95 (2026-07-18, DividerWdgt.isDivider role query retired 2 `instanceof DividerWdgt` in removeConsecutiveLines); 95->93 (2026-07-30, banking a gain already landed; fg critique surfaced it); 93->88 (2026-08-04, layout spec-family arc: the enum dispatch + 5 spec-class instanceofs became capability queries); 88->87 (2026-08-09, banking a gain already landed; surfaced by the comments-audit stink run)
    ...
    re: /\binstanceof\b/ },
  { id: 'positional-hole', baseline: 1,   // seeded 2026-08-15 at THIS engine's own count (51); tightened 51->30 (plan P2, the text family), 30->28 (P3, the button family), 28->25 (P5, the stragglers), 25->1 (P6, the METHOD families), same day. P4 moved it by zero and that was correct: every hole it removed spans MULTIPLE lines and this regex needs two `undefined`s adjacent on ONE. The ONE survivor is ActivePointerWdgt's `target.add` — the polymorphic 8-override add family, owner-gated as its own change (docs/plans/constructor-parameter-conformance-plan.md §7)
