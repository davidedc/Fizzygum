...
const BASELINE_U_QUERY = 121;      // 152 at rule birth; T2 deleted the vestigial buildSubwidgets hooks; T5's renames took 1 more off; tightened 150 -> 148 on 2026-07-15 to lock in a gain a later arc had already banked (fg critique surfaced it); 148 -> 147 on 2026-07-22 with the invalidation-privacy rename (the changed family left the public-query census); 147 -> 143 on 2026-07-22 with the extent+move pair fusion (_applyBounds/setBounds one-shots); 143 -> 132 on 2026-07-30 banking gains arc 3 had already landed without tightening (it re-homed the test-only members out of src into Fizzygum-tests' *TestSupport classes and retired the »>> region mechanism, deleting whole experimental classes — both take methods out of this census), surfaced by this gate's own NOTE; 132 -> 131 on 2026-08-04 (layout spec-family arc: the division-stack public face divisionBox() gained real external callers, leaving the self-only census); 131 -> 127 on 2026-08-05 (follow-ups arc close, locking a drift the gate's own NOTE had surfaced: editLayout gave ensureLoaded a real src caller — its allowlist entry deleted the same day — and earlier arcs' renames had already banked the rest); 127 -> 126 on 2026-08-17 (connector P8: SliderWdgt.updateSpecs became _updateSpecs once the scroll frame stopped PUSHING four numbers into its bars — this gate's own NOTE is what flagged that it had stopped being external API); 126 -> 125 on 2026-08-17 (connector P2: Widget.principalPin gained real external callers — the bind gesture asks a candidate TARGET for its principal pin, where before only exportedValue asked it of itself); 125 -> 124 on 2026-08-19 (graph-edges §4.3: PatchNodeWdgt.fireOutputToTarget folded into its one caller reactToTargetConnection — the wire self-heal made the _fireConnection chain effectful, and this gate's [U] reclassification surfaced that the indirection had always been self-only); 124 -> 123 on 2026-08-24 (pointer-events P2: ActivePointerWdgt.isThisPointerNonFloatDraggingSomething gained a real external caller — the pointercancel witness test asks the hand, from a macro, whether a non-float drag was live when the abort arrived); 123 -> 121 on 2026-08-26 (command-panel unification P4: the Draw palette re-based onto the toolbar family, dissolving the radio construction — RadioButtonsHolderWdgt.whichButtonSelected and PaintToolbarWdgt.isToolPressed were both @-self-only public queries and went with it)
