...
def generateTestsManifests(testsDirectory):
    ...
    if not (args.homepage or args.notests):
        ...
        filenames2 = []
        onlyDirectories = [f for f in os.listdir("../Fizzygum-tests/tests/") if os.path.isdir(os.path.join("../Fizzygum-tests/tests/", f))]
        for filename in onlyDirectories:
            filenames2.append(filename)
            print("will add to test manifest: %s" % (filename))
        filenames2 = sorted(filenames2)

        for filename in filenames2:
            manifest = manifest + "testsManifest.push('"+filename+"');\n"

    # 'build/indexWithTests.html'
    with codecs.open("../Fizzygum-builds/latest/js/tests/testsManifest.js", "w", "utf-8") as f:
        f.write(manifest)
    print("... finished creating the test manifest")

def generateTestsAssetsManifests(testsDirectory):
    ...
    with codecs.open("../Fizzygum-builds/latest/js/tests/testsAssetsManifest.js", "w", "utf-8") as f:
        f.write(manifest)
    print("... finished creating the test *assets* manifest")
...
def main():
    ...
    filenames = sorted(filenames + sorted(glob("src/synthetic-events" + "/*.coffee")))
    ...
    if not args.keepTestsDirectoryAsIs:
        generateTestsManifests(DIRECTORY_WITH_TEST_FILES)
        generateTestsAssetsManifests(DIRECTORY_WITH_TEST_FILES)
