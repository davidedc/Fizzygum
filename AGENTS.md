...
- **Tests are SERVED through a symlink, never copied.** The build points `../Fizzygum-builds/latest/js/tests` at the sibling `Fizzygum-tests/tests` directory (`ln -sfn`), so a test edit, a freshly captured reference, or a brand-new test directory is live **with no rebuild** — only framework/harness `.coffee` changes need one. `--homepage` / `--notests` builds carry no `js/tests` entry at all. ⚠⚠ Never aim a recursive delete at that path: `rm -rf <link>/` (trailing slash) deletes the REAL suite, and `find -L` descends into it; the build owns it via one `ln -sfn` and one guarded `[ -L p ] && rm -f p`.
