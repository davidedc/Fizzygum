...
class StringMorph3 extends Morph
  ...
  constructor: (
      @text = (if text is "" then "" else "StringMorph3"),
  ...
  # needed because we can't edit a TextMorph3 "in place"
  ...
  fitMenu: (a,targetMorph)->
    ...
    if @fittingSpecWhenBoundsTooLarge == FittingSpecTextInLargerBounds.SCALEUP
      menu.addMenuItem tick + "←→ expand to fill", true, @, ""
      menu.addMenuItem space + "←☓→ don't expand to fill", true, @, "togglefittingSpecWhenBoundsTooLarge"
    else
      menu.addMenuItem space + "←→ expand to fill", true, @, "togglefittingSpecWhenBoundsTooLarge"
      menu.addMenuItem tick +  "←☓→ don't expand to fill", true, @, ""

    if @fittingSpecWhenBoundsTooSmall == FittingSpecTextInSmallerBounds.CROP
      menu.addMenuItem tick + "→⋯← crop to fit", true, @, ""
      menu.addMenuItem space + "→← shrink to fit", true, @, "togglefittingSpecWhenBoundsTooSmall"
    else
      menu.addMenuItem space + "→⋯← crop to fit", true, @, "togglefittingSpecWhenBoundsTooSmall"
      menu.addMenuItem tick + "→← shrink to fit", true, @, ""
  ...
  developersMenu: ->
    ...
    menu.addMenuItem "fit ➜", false, @, "fitMenu", "how the text and the container fit with each other"
  ...
  disableSelecting: ->
    ...
    @clearSelection()
    @mouseDownLeft = StringMorph3::mouseDownLeft
    delete @mouseMove
