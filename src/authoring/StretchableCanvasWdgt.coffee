...
class StretchableCanvasWdgt extends CanvasWdgt
  ...
  getContextForPainting: ->
    ...
    # reach back with a _changed() of their own (widget-citizenship point 2);
  ...
  # (a line primitive stays on the raster's owner -- see CanvasWdgt.drawLine)
  ...
  _reactToChildDropped: (droppedWidget) ->
    @_paintImage droppedWidget.position(), droppedWidget.fullImage(forceShadow: true)
