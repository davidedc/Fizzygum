class TypewriterIconAppearance extends IconAppearance
  ...
  paintIntoAreaOrBlitFromBackBuffer: (aContext, clippingRectangle, appliedShadow) ->
    ...
    # deliberately NO useLogicalPixelsUntilRestore(): everything below is drawn in
    ...
    # widget's bounds are integer logical px (integer-placement policy), so these
    # products are exact integers.
    @_paintPixelTypewriter aContext,
  ...
  # integer row spans [x0, width] of a pixel disc of diameter k
  _pxDiscRows: (k) ->
    c = k / 2
    for j in [0...k]
      ry = j + 0.5 - c
  ...
  # a stadium/capsule (the spacebar): straight middle, disc-row caps
  ...
  _paintPixelTypewriter: (ctx, x0, y0, wDev, hDev) ->
