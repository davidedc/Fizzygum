...
class SheetModel
  ...
  # a plain data class, but it lives in the widget tree and must be DEEP-COPIED per sheet (so a
  # duplicated sheet gets its OWN model, not a shared reference) — handled automatically by the
  # Duplicator's generic walk, with no per-class opt-in needed. @cells (a Map) copies via the
  # Duplicator's own Map handler; @sheetWidget maps to the copied widget through the copier's
  # identity tracking.
