...
class HorizontalMenuPanelWdgt extends PanelWdgt
  ...
  add: (aWdgt, position = nil, layoutSpec = LayoutSpec.ATTACHEDAS_FREEFLOATING, beingDropped, unused, positionOnScreen) ->
    ...
    if aWdgt.attachesToScrollFrameDirectly?()
      ...
      if !(aWdgt.isGlassBoxWrapper?())
        ...
        glassBoxBottom._applyMoveTo @topLeft().add new Point @internalPadding, @internalPadding
        ...
        if aWdgt.isTextSizedGlassBoxItem?()
          ...
          glassBoxBottom._applyExtent new Point aWdgt.width(), @thumbnailSize
        else
      ...
      if dropSlot?
        super aWdgt, dropSlot, layoutSpec, beingDropped
      else
  ...
  _reLayoutSelf: ->
    ...
    for i in [0...countOfItems]
      ...
      childrenNotHandlesNorCarets[i]._applyMoveTo (startingPoint.add new Point widthLayingDown, (@height()-childrenNotHandlesNorCarets[i].height())/2).round()
