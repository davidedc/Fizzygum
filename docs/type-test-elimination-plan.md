...
- ☐ **menu-item unselect** — `MenuWdgt.unselectAllItems:~231` `if item instanceof MenuItemWdgt then item.state = item.STATE_NORMAL` (the 3rd `instanceof MenuItemWdgt`, a state-reset not glass-box layout) → behaviour-move `item.unselect?()` (MenuItemWdgt sets its own STATE_NORMAL).
