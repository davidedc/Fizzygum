...
**hard-fail capstone gate**. Full model: `docs/layout-system-architecture-assessment.md` §2.7.
...
`eoc-production-probe.js`). It is **currently at zero**. Companion docs: `docs/end-of-cycle-flush-inventory.md` (the
audit history), `docs/end-of-cycle-flush-drawdown-plan.md` (playbooks/patterns), `docs/end-of-cycle-flush-final-records
...
- `docs/layout-system-architecture-assessment.md` **§2.7** — the flush model, the three faults + discriminator, the
  coalescing/`*Coalesced` API.
- `docs/end-of-cycle-flush-inventory.md` — the by-action audit history (incl. the 2026-06-26 `childRemoved` banner =
  the precedent in §4, and the basement-orphan reasoning).
- `docs/end-of-cycle-flush-drawdown-plan.md` — worked playbooks, code patterns, the verification protocol, the tricks
  (disable-probe / stack-probe / narrowest-guard).
- `docs/end-of-cycle-flush-final-records-plan.md` — the arc that reached zero + shipped the capstone gate this targets.
