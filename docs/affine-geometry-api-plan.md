...
- **LAYOUT-BOX family** (the existing names): `width()/height()/extent()/bounds/boundingBox()/
  ...
  (`docs/affine-transforms-plan.md` §4.1 "virtual plane") — layout, settle, arrange, and content
...
damage/hit-lane machinery (`docs/affine-transforms-plan.md` §4.11) and predate this law. They are
...
1. 4D-1's drop placement assumed `payload.center()` is the payload's visual centre — broken once
   ...
   note in `docs/affine-transforms-plan.md`).
...
(`docs/affine-transforms-plan.md` 4D-2a key finding: scalar rotations commute; no matrices).
...
- `docs/affine-transforms-plan.md`: add a short **§4.13 "Public geometry API contract"** that
...
0. **Pre-flight:** confirm the affine arc units this plan is sequenced AFTER are landed (top
   banner of `docs/affine-transforms-plan.md`); re-run the §2 collision greps; check whether
