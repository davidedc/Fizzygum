...
- **What shipped this campaign** (each a CONVERT / ELIMINATE / new-infrastructure step, all byte-identical, all
  ...
  - The whole **`*Coalesced` + declared-coalescing infrastructure**: `Widget.setMaxDimCoalesced` (the first and only
    ...
    reporting in `recalculateLayouts`, and the measurement **harness** (`docs/coalescing-measurement.md`).
...
- **Earlier-campaign shipped work** (context; do not redo): teardown (`destroy`/`close`/`fullDestroy`), the
  ...
  is a detached/orphan subtree). Full history: `docs/end-of-cycle-flush-inventory.md`.
...
warrants coalescing (`docs/coalescing-measurement.md`: max≈1 → don't bother; max≫1 → coalesce).
...
Full playbooks + worked case studies: `docs/end-of-cycle-flush-drawdown-plan.md` §3–§3d (the `destroy`, single-settle,
...
- **COALESCE / DECLARE:** wrap the stream's core through `_coalescedDeclare` behind a `*Coalesced` public entrypoint
  (pattern: `setMaxDimCoalesced`) — but FIRST measure it's worth it (`docs/coalescing-measurement.md`: max≈1 → just use
...
- Per resolution: flip the row + new total + dated banner in `docs/end-of-cycle-flush-inventory.md`, and update memory
...
- **Up-to-date companion docs:**
  - `docs/layout-system-architecture-assessment.md` **§2.7** — the CONCEPTS: flush model, the three faults +
  ...
  - `docs/end-of-cycle-flush-drawdown-plan.md` — the worked PLAYBOOKS (§3–§3d), code PATTERNS (§5), VERIFICATION (§6),
    TIPS (§8), file map (§9). (§1/§2 there now point into §2.7.)
  - `docs/end-of-cycle-flush-inventory.md` — the full by-action AUDIT HISTORY + verdicts.
