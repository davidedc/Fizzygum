...
approach** (see `docs/retire-adjustingContentsBounds-via-text-measure-plan.md`, the ⛔ VERDICT block). The corrected
...
- **Phase E is the biggest determinism surface.** Replacing the seam changes re-fit timing across many gestures. Migrate
  ...
  (`docs/softwrap-deferred-layout-conversion-plan.md`) is reversal-heavy — expect care.
...
- **`docs/layout-system-architecture-assessment.md` is the owner's parallel-work — READ it, do NOT edit it.**
...
- **The authoritative analysis (READ FIRST, owner's WIP — kept current with this plan):**
  `docs/layout-system-architecture-assessment.md` — §2.3 (fixpoint), §2.4 (read-back root), §2.5 (two sizing
...
- **The corrected falsification record (why the naive measure fails):**
  `docs/retire-adjustingContentsBounds-via-text-measure-plan.md` (⛔ VERDICT block) + memory
  `fizzygum-adjustingcontentsbounds-flag`.
- **Context:** `docs/deferred-layout-OVERVIEW.md` (render/layout separation, the in-pass-enqueue/off-pass-invalidate
  seam, the `elasticity 0` cycle-break §5), `docs/softwrap-deferred-layout-conversion-plan.md` (§5 — the reversal-heavy
