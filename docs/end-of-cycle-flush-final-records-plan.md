...
- **Earlier-campaign shipped work** (context; do not redo): teardown, contained-text API path, caret/handle wasted
  ...
  `docs/end-of-cycle-flush-inventory.md` (its 2026-06-27 banner + the "Current numbers" line are UP TO DATE — read them).
...
**The model in 5 lines (full version: `docs/layout-system-architecture-assessment.md` §2.7).** The engine drains
...
- **T14 — New plan/doc files are UNTRACKED** and silently excluded from a `git add <listed files>` commit — fold into
  ...
  pre-existing `docs/end-of-cycle-flush-drawdown-plan.md` / `docs/layout-system-architecture-assessment.md` mods and the
  untracked `docs/end-of-cycle-flush-endgame-plan.md`, are uncommitted — ask the owner what to do with them.)
...
- Per resolution: flip the relevant row + new total + a dated banner in `docs/end-of-cycle-flush-inventory.md`, and
...
- **Up-to-date companion docs:**
  - `docs/end-of-cycle-flush-inventory.md` — the full by-action AUDIT HISTORY + verdicts; its **2026-06-27 banner** +
    "Current numbers" line (5 records) are CURRENT.
  - `docs/layout-system-architecture-assessment.md` **§2.7** — the CONCEPTS: flush model, the three faults + discriminator,
    the coalescing model + `*Coalesced` API.
  - `docs/coalescing-measurement.md` — the measurement HARNESS + the divider-drag case study (the verdict rule).
  - `docs/end-of-cycle-flush-drawdown-plan.md` — the worked PLAYBOOKS, code PATTERNS, file map.
  - `docs/end-of-cycle-flush-endgame-plan.md` — the PRIOR endgame plan (covered groups 1–5; groups 1+2 are now SHIPPED
