> **ARCHIVED — COMPLETE (2026-07-17 restructure).** Campaign COMPLETE per its own bottom line; superseded by deferred-layout-OVERVIEW.md on any conflict.
...
- **(b) the CYCLE** — `WorldWdgt.doOneCycle → recalculateLayouts` until-loop (`_recalculateLayoutsCore` ~:876, loop
  ~:885), draining `widgetsThatMaybeChangedLayout` via `_reLayout` until convergence.
...
- **The twin's outside-pass container re-fit now DEFERS (2026-06-21).** `_refreshScrollPanelWdgtOrVerticalStackIfIamInIt`
  ...
  `Widget.newParentChoice*` (direct `_positionAndResizeChildren`/`_reLayoutChildrenAndScrollbars`) in 3; the `reInflating`-coupled
  direct `_reLayoutChildren` in collapse (4, must stay synchronous); the `@_reLayoutSelf()` in content-edit + soft-wrap (5,
...
- **The drag/drop gesture re-fits now DEFER (2026-06-21) — family 2 below.** The four gesture seams
  ...
  next `doOneCycle` (each container's `_reLayout` is `super; @_reLayoutChildren`). Byte-identical (165/165
  dpr1+dpr2+WebKit, smoke OK). LEFT synchronous (correct, per the mapping Workflow): the cascade SINK is each
  container's deferred `_reLayout` → `_reLayoutChildren` (the synchronous `childGeometryChanged` climb arm was retired by
  the capstone and that orphaned method deleted), `_reLayOutAfterContainedPanelChange`/`_reLayoutChildrenAndScrollbars` (absorb
...
- **The `newParentChoice*` dev-menu re-fits now DEFER (2026-06-21) — family 3 leftover.** `Widget.newParentChoice`
  and `newParentChoiceWithHorizLayout` ("attach this selected widget to me") used to call `@_reLayoutChildrenAndScrollbars?()`
  synchronously right after a self-settling `@add`. Now the call site is the 2-way deferred form (mirrors
  `reactToDropOf`): an existence check `if @_reLayoutChildrenAndScrollbars?` does the old `?()`-soak's job (no-op off a
  ...
  because `_reLayoutChildrenAndScrollbars` IS `@_reLayoutChildren`, the very method `_reLayout` (`super; @_reLayoutChildren`)
  runs next cycle — and `@add` already flushed `@desired*`, so `super` is a no-op. Byte-identical (165/165
  dpr1+dpr2+WebKit, smoke OK). The shared `_reLayoutChildrenAndScrollbars`/`_reLayOutAfterContainedPanelChange` pair stays
...
1. **Scroll-input handlers (ScrollPanelWdgt) — VERDICT: LEAVE SYNCHRONOUS (assessed 2026-06-21).** Synchronous
   `_positionAndResizeChildren`/`_reLayoutScrollbars` straight from input handlers: `wheel` (~:666), `mouseDownLeft`
   ...
   does not apply; deferral would just relocate the identical `_positionAndResizeChildren`+`_reLayoutScrollbars` to
   ...
   (which polices immediate mutators) does not need them; and they call `_positionAndResizeChildren` directly, never via
   the seam's `_reFittingContents` branch, so they are independent of the counter retirement. (`scrollCaretIntoView`
   additionally has an internal read-back — `_positionAndResizeChildren` at ~:642 must run BEFORE the caret math reads
...
2. **Drag/drop re-fit cascades — DONE (DEFERRED 2026-06-21, `1e5d3745`; see "Already compliant" above).** The gesture
   ...
   left synchronous BY DESIGN: the cascade SINK (each container's deferred `_reLayout` → `_reLayoutChildren`; the
   synchronous `childGeometryChanged` climb arm was later retired by the capstone and the orphaned method deleted),
   `_reLayOutAfterContainedPanelChange` / `_reLayoutChildrenAndScrollbars` (the absorb return-value contract). [`PanelWdgt.childRemoved`
...
3. **Menu actions** (the twin-mediated part is **DONE — deferred `1caea690`**). `VerticalStackLayoutSpec.setAlignmentToLeft/Right/Center`/`setElasticity`/`setWidthOfElementWhenAdded`
   ...
   compliant" below): the post-`@add` synchronous `@_reLayoutChildrenAndScrollbars?()` is now the 2-way deferred form
...
4. **Collapse / uncollapse** (the twin-mediated part is **DONE — `1caea690`**). `WindowWdgt.childCollapsed`/`childUnCollapsed`
   via `Widget.collapse`/`unCollapse`. **REMAINING:** the `reInflating`-coupled direct `@_reLayoutChildren()` (must stay
...
5. **Content-edit / soft-wrap — VERDICT: LEAVE SYNCHRONOUS (assessed 2026-06-21).** The twin-mediated container re-fit
   is **DONE — `1caea690`**. The widget's own `@_reLayoutSelf()` re-wrap was given a full design pass (mapping Workflow +
   read-audit + adversarial verify) → **leave synchronous in its entirety**: (a) `TextWdgt.rawSetExtent`'s re-wrap is
   the in-pass APPLY the base `_reLayout` depends on (family-8; converting throws); (b) `setSoftWrap` wrap-OFF `@_reLayoutSelf()`
   ...
   `@_reLayoutSelf()` is redundant ONLY in a text-wrapping scroll panel, and **that case is blocked by a same-cycle caret
   ...
   `TextWdgt._reLayout` (no-go — flips `implementsDeferredLayout`, fires a redundant re-wrap, buys nothing). Reward is
...
6. **Slider family — VERDICT: LEAVE SYNCHRONOUS (assessed 2026-06-21).** `SliderWdgt.setValue` (:117/119, **mid-drag**
   ...
   (config setters), `rawSetExtent` (:74). ~13 synchronous `_reLayoutSelf` sites. **No pattern surface:** `SliderWdgt.@_reLayoutSelf()`
   resolves to the EMPTY `Widget._reLayoutSelf` base (no `SliderWdgt`/`CircleBoxWdgt` override) — a no-op; the `@button._reLayoutSelf()`
...
7. **LabelButton — VERDICT: LEAVE SYNCHRONOUS (assessed 2026-06-21).** `alignCenter`/`alignLeft`/`setLabel` (:104/110/115)
   → synchronous `_reLayoutSelf` from a menu/button. Already compliant in substance: the `_reLayoutSelf` re-centers only the
...
8. **THE STRUCTURAL ROOT — `rawSetExtent` runs `_reLayoutSelf`.** Base `Widget.rawSetExtent` (~:1520) =
   `silentRawSetExtent` + `changed` + **`@_reLayoutSelf()`**, unconditionally (no pass guard; base `_reLayoutSelf` is empty, so
   it bites only where `_reLayoutSelf`/an override runs layout). This is INTENDED as the in-pass synchronous-APPLY
   mechanism (`rawSetWidthSizeHeightAccordingly` relies on it, Widget.coffee ~:706), but it makes `rawSetExtent`'s
   `_reLayoutSelf` an off-settle residual at every **off-pass** call site — collapse handlers (WindowWdgt:259-260), the
   hand's `drop`/`determineGrabs` (ActivePointerWdgt:252/840), and the Stretchable*/Slider/TextWdgt `rawSetExtent`
   overrides that add `@_reLayout()`/`@_reLayoutSelf()`. This underlies families 4/6/7 and is the hardest to convert
...
- **The transport/drag path is mostly compliant.** Handles use public setters; grab/drop use `add`/`setExtent`/
  `fullMoveTo`. The genuine drag residuals are only the `reactToGrabOf`/`reactToDropOf` `_reLayoutChildren` cascades
  (family 2) and the hand's `rawSetExtent`→`_reLayoutSelf` (a no-op for the hand). The deferred clamp `fullMoveWithin`
...
- **lint [E] tightening + retiring `world._reFittingContents` — ✅ BOTH RESOLVED 2026-06-21 (capstone `a7463bbc`). The
  ...
  synchronous `childGeometryChanged` arm retired + the orphaned method deleted), leaving only TERMINAL `_reLayoutChildren`
  applies that are sanctioned like `_reLayoutSelf`/`_reLayout`. See `deferred-layout-capstone-execution-plan.md` (RESULT-2 +
  Part B). _(Original pre-capstone reasoning retained below for history.)_ `check-layering.js` rule [E] currently polices only
  `invalidateLayout` from `raw*`/`silent*`/`fullRaw*`; tightening it to also forbid synchronous `_reLayoutChildren`/
  `childGeometryChanged`/`_reLayoutSelf` needs **family 8** (the `rawSetExtent→_reLayoutSelf` structural root) converted first —
  it is the load-bearing blocker. Retiring the `_reFittingContents` counter is blocked independently: the counter's
  synchronous middle arm stays load-bearing as long as ANY `_reLayoutChildren` runs outside a recalc pass with a nested
  raw change, and two such roots WERE believed un-deferrable — **`WindowWdgt.add`** then called
  `@_reLayoutChildren()` before its `super` flush [**⚠ STALE: that pre-fit was REMOVED/deferred by the capstone
  (WindowWdgt.coffee ~:208-213); see `deferred-layout-OVERVIEW.md` §4/§11**], and **`WindowWdgt.childUnCollapsed`** runs a
  `reInflating`-coupled `@_reLayoutChildren()` whose `_positionAndResizeChildren` reads `!@reInflating` mid-run (WindowWdgt:157).
  So the counter cannot be deleted ahead of resolving the structural root + those un-deferrable sites + the
  `_reLayOutAfterContainedPanelChange` return-value contract. (A naive in-pass seam removal already broke 7 tests — the
...
- **Families 1 (scroll-input), 6 (Slider), 7 (LabelButton): LEAVE SYNCHRONOUS** (verdicts above). Family 1 is the
  ...
  wrong problem class (self-container, not child→container). Family 6 has no pattern surface (empty-base `_reLayoutSelf`).
  Family 7 is already compliant in substance. None blocks the capstone.
- **`BoxWdgt.choiceOfWidgetToBePicked`: dead code** — leave it.
- **Soft-wrap `_reLayoutSelf` (family 5 remainder): its own dedicated hard arc** — `softwrap-deferred-layout-conversion-plan.md`.
- **The capstone — retire `_reFittingContents` + lint [E]: ✅ COMPLETE 2026-06-21 (`a7463bbc` + `66a41a57`).** The C2
  ...
  eliminated the CLIMB vector and orphaned `childGeometryChanged` (deleted); the two `rawSetExtent→_reLayoutChildren`
  sites are TERMINAL applies identical to the sanctioned `_reLayoutSelf`/`_reLayout`, so forbidding the name was declined as
