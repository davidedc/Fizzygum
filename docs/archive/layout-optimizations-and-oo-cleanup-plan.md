> **ARCHIVED — COMPLETE (2026-07-17 restructure).** All Tiers A-J landed and pushed; no live items remain.
...
5. **The stack's per-child measure is written 3× with a THREE-WAY clamp/round divergence**
   ...
   `docs/archive/fractional-widget-bounds-investigation-plan.md`). That guard checks the *committed* `@bounds`, so it does
...
(b) `docs/archive/layout-system-architecture-assessment.md` — update the five "[A]–[M]"/"[A]–[N]" rule-range mentions to
...
See `docs/archive/oo-smells-refactoring-backlog.md` / `docs/archive/god-class-decomposition-plan.md`. *(The cache-region debug
