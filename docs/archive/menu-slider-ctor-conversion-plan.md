> **ARCHIVED — COMPLETE (2026-07-17 restructure).** COMPLETE 2026-07-12 — all 4 sites converted, exemption count 0, byte-identical.
...
- The all-constructors-settle campaign (2026-06-30, `08bbb29d`; doc
  `docs/archive/all-constructors-settle-plan.md`) converted the 13 inline-building constructors it
...
- **Ctor-param binding**: CoffeeScript binds subclass `@`-ctor-params only AFTER `super()` —
  ...
  `check-constructors-build.js` header + `docs/archive/all-constructors-settle-plan.md`). PromptWdgt
...
2. Sync `docs/architecture/lint-and-static-checks.md` (constructor-build row mentions "4 menu/slider-
   family ctors carry it" — update) and `docs/archive/all-constructors-settle-plan.md` (add a
...
- P4 closeout: ✅ 2026-07-12 — gate BUILD-regex comment updated (exemption clause →
  conversion pointer); `docs/architecture/lint-and-static-checks.md` §3 row + §long-form synced (count
  ZERO); `docs/archive/all-constructors-settle-plan.md` completion note added (the `__add` blind
