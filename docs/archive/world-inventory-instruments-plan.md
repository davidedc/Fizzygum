...
`canvases.count`/`canvases.pixels` · `strings.count`/`strings.chars` ·
`dom.headScripts`/`dom.bodyChildren` · `instances.<Class>` (= `instances.size`) ·
`statics.<Class>.<field>` (size of each own static Set/Map/Array) ·
...
## §5 Spikes (Phase 1 — scratch only, no src edits)
