> **ARCHIVED — COMPLETE (2026-07-17 restructure).** Spikes + Phases 1-6 all LANDED 2026-07-06 (gates green each phase); pushed per project ledger.
...
- Gates: gauntlet (expect NO reference changes — the legacy descriptor must reproduce today's pixels; verify,
  ...
  snapshot API + envelope format is documented in `docs/architecture/serialization-duplication-reference.md` (§11
...
> ↪ The cut pencil↔eye piece now has its OWN dedicated plan: **`docs/archive/pencil-eye-edit-mode-toggle-plan.md`**
...
- **Deserialization** of old snapshots ignores any stored `internal` (derive from parentage on load). VERIFY vs
  `docs/architecture/serialization-duplication-reference.md`. The `internal` / `alwaysShowInternalExternalButton` constructor
...
```coffee
    # »»»» THROWAWAY S1/S2 PROBE (docs/archive/drag-embed-implementation-plan.md §2) — NEVER COMMIT »»»»
