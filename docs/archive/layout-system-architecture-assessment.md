> **ARCHIVED — COMPLETE (2026-07-17 restructure).** Living layout ledger, re-grounded 2026-07-16; all tracked campaigns closed as of 2026-07-17.
...
per-class composite-relay layer fully deleted** (`docs/archive/ordered-downwalk-stage-b-plan.md` is the complete record: §8 =
...
endgame** (2026-07-17, `docs/archive/upedge-endgame-plan.md`) then retired the residual up-edge re-visits at the root too —
...
*within* a painted frame; see `docs/archive/hover-resync-after-flush-plan.md`). The caret's **scroll-follow** — "bring the caret into
...
**⇄ CAMPAIGN CLOSED (2026-07-16/17, `docs/archive/sizing-model-unification-plan.md` — the arc this finding called
...
`docs/archive/sizing-model-unification-plan.md` §9).** The three listed gaps closed at the root: the proportional cycles died
...
**ZERO — the baseline is EMPTY** (up-edge endgame V1, 2026-07-17, `docs/archive/upedge-endgame-plan.md` §9: the
...
- **orphan / construction** — an off-world widget (root is neither `world` nor `world.hand` — `isOrphan`,
  ...
  *(A sharper corollary, now in its mature form after the all-constructors-settle campaign
  (`docs/archive/all-constructors-settle-plan.md`, 2026-06-30): EVERY constructor builds its children in a non-settling core
...
- **The disable-probe** — the convert-vs-eliminate decider. No-op the suspected-redundant re-fit, build, run the suite:
  ...
  **one-line ELIMINATE**, no convert at all. The lesson generalizes: a leak that *presents* as a public-mutator convert
  ...
  `_*NoSettle` core — 2026-07-04, `docs/archive/disable-editing-family-convert-plan.md` — because ScrollPanelWdgt's ELIMINATE did
...
`docs/archive/ordered-downwalk-stage-b-plan.md` (§8 = B1 two-flag scaffold + B2 root-down visitation + B3 the frame-changed
...
- **Per-frame cycle:** `WorldWdgt.doOneCycle` ~:1350 · `playQueuedEvents` ~:1269 (whole-queue drain ~:1274,
  ...
  under a stationary pointer against SETTLED geometry; def `ActivePointerWdgt` ~:920; `docs/archive/hover-resync-after-flush-plan.md`)
...
2. **Keep it to ONE pass.** A container fits its content in a *single* settle visit. You get this for free if you obey
   ...
   container mid-arrange (§4.1). Since the up-edge endgame (2026-07-17, `docs/archive/upedge-endgame-plan.md` §9) the
