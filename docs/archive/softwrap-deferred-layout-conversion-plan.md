> **ARCHIVED — COMPLETE (2026-07-17 restructure).** REFERENCE (historical) — soft-wrap ASSESSED 2026-06-21, verdict LEAVE SYNCHRONOUS; superseded by deferred-layout-OVERVIEW.md
...
>   change — the full plan is in **`docs/archive/deferred-layout-path-a-design.md`**, which supersedes the
...
2. **Path A proper — pending-aware accessors** (generalize `pickUp` **and** `fullRawMoveWithin`, and
   ...
   `docs/archive/deferred-layout-path-a-design.md`. NB: the blanket "pending-aware accessors" form of this was
...
- **Path A's risk is breadth AND conflicting read semantics** (accessors are everywhere; the hand drives
  ...
  `docs/archive/deferred-layout-path-a-design.md`.
...
`setSoftWrap` (byte-identical). The campaign tracker is `docs/archive/oo-smells-refactoring-backlog.md`; the
deferred-pattern audit summary lives under its Phase 7. The God-class arc (Phase 6, COMPLETE) is
`docs/archive/god-class-decomposition-plan.md`. The byte-exact contract is `Fizzygum-tests/DETERMINISM.md`.
