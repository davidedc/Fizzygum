...
**PROGRAM CLOSED 2026-08-27.** All five plans (1, 2, 3, 3.5, 4) are EXECUTED, CLOSED and
archived; the tail ledger below holds ZERO open rows (every deferred item carries a
destination — a `docs/BACKLOG.md` row or a closing plan's record). The living truth this
program produced is in the architecture corpus (`input-and-gestures.md`,
`viewports-and-planes.md`, `widget-authoring-guidelines.md`, the macros CLAUDE docs, the
tests-repo reference grammar); the per-plan as-executed logs are the archived plans' STATUS
boxes. Kept verbatim below as the program's record.
...
- Plan 2 `pointer-events-plan.md` — **EXECUTED AND CLOSED 2026-08-24, same day it was authored** (archived: `../archive/pointer-events-plan.md`; P0 probes GO → P1 the swap pixel-identical → P2 cancel witnessed → P3 sweep + gauntlet 18/18 + homepage; tail EMPTY). Suite 318.
- Plan 3 visual wave — **EXECUTED AND CLOSED 2026-08-24/25** (archived: `../archive/single-geometry-visual-wave-plan.md`,
  STATUS box = the as-executed log; wave commit Fizzygum `5a45a135` / tests `530b49d4e`, PUSHED:
  the single geometry landed suite-wide, 321 tests at 1920×880 / Automator 0.3.0, presuite 321/0,
  gauntlet 18/18, homepage OK; P4 absorbed into the wave; P5 docs+close ran at the close). The
  plan file moves to `docs/archive/` with the close commit.
- Plan 3.5 command-panel unification (T1 as expanded) — **EXECUTED AND CLOSED 2026-08-25/26**
  (archived: `../archive/command-panel-unification-plan.md`, STATUS box = the log; the
  command model + the `User` model landed; two design premises falsified in execution and
  re-framed, dated).
- Plan 4 gesture grammar + finger harness — **EXECUTED AND CLOSED 2026-08-26/27** (archived:
  `../archive/gesture-grammar-and-finger-harness-plan.md`, STATUS box = the log). The grammar
  in the hand (hold-as-right-click, un-armed-drag-scrolls, chrome-by-capability, a scroll
  dispatches no click, hover dissolution + pointer absence, T7's keyboard), the finger
  harness (L1 touch verbs, the intent translation under unchanged verb names, the
  `…/ceilPixRatio_<N>/finger/` reference axis, ~1,862 finger references at dpr 1+2 per
  OD2(a)), and the `finger` gauntlet leg (wave D, WebKit+dpr2). Suite 329 mouse + 318
  finger, gauntlet 19/19, presuite byte-identical throughout (the mouse world never moved:
  the two declared single-image recaptures are the whole pixel cost). T6/T7 closed; T18
  filed per OD4 with the G2 pairing; T4/T5 filed.
...
the `isMenu?()` check at its `processPointerDown`, `hierarchyOfClickedMenus`): Spine I rewrites
...
| **C6** | **Close button and resizer show iff `isFreeFloating()`** (the predicate the handles already use). A host that owns placement (edge spec, stack spec, window-content spec) owns membership — you leave by dragging out. Consequence accepted by the owner: **an internal window (window as window content) loses its close button**. Cards in stretchable containers keep close (follower spec ⇒ free-floating). Right-click → close remains the universal fallback — and closes the NESTED frame alone: the `unless the content is itself a frame` conjunct in `Widget._closeNoSettle` (and its twins in `_moveToTrashNoSettle` / `_wouldTrashSeverAnything`) STAYS. ⚠ Amended 2026-08-23 (owner ruling B at Plan 1 P2): the original text deleted that conjunct as if it were about the button; it is about close SEMANTICS — without it "close inner" redirects to the outer and the whole assembly leaves, which falsifies the fallback sentence and `FrameWdgt._beforeChildClosed`'s reset-to-empty behaviour. | A close control you can hit while reaching for a grip is the thing to avoid; "no toolbar exception" — a docked document loses close for the same reason. |
...
| **C13** | **Bar orientation**: an expanded docked frame's bar runs ACROSS the strip at its leading end (vertical at the left end of a top/bottom dock, horizontal at the top of a left/right dock); a **collapsed frame is its bar, spanning the axis it keeps** (for a dock: along the edge — a full-width band for a top dock, a full-height sliver for a side dock). No flip: the frame's shape changes and the bar is laid out for the shape. **Text only on horizontal bars.** Expanded dock: the PAYLOAD receives its declared `dockThickness` exactly, the band adds only its own body margin, and the BAR adds nothing to the thickness (it crosses at the leading end) — amended 2026-08-23 at Plan 1 P5: the original 'band = dockThickness exactly' was written under the old slot model, where the strip WAS the payload, and taken literally it clips the payload by the frame's margin; collapsed = bar thickness (not 0 — there must be something to tap). Amended 2026-08-24 (owner rulings at the Plan 3 P2 probe): **the title is GRADED, never garbled** — a bar renders its full title when it fits; ellipsized only while the visible prefix keeps ≥ `barTitleEllipsisMinFraction` (0.7) of the characters AND ≥ `barTitleEllipsisMinChars` (5); otherwise NO title (`"World…"`/`"t…"` never appear). Title text is vertically centred in the bar and horizontally centred in the bar's full width; **a textless bar centres its piece group along its axis** (a side-dock's horizontal bar centres the collapse icon horizontally; a top-dock's vertical bar centres it vertically). | Qt `DockWidgetVerticalTitleBar` precedent; compactness without a new skin. |
...
| **C17** | **The whole bar of a COLLAPSED frame is tappable to expand it** (not just the collapse button) — ruled by the owner 2026-08-23 (during Plan 1 P3). Applies to every manifestation whose collapsed form is its bar (C13): a window's collapsed strip, a docked sliver. A tap on an EXPANDED bar keeps today's meaning (drag = move, title tap pins a transient frame); only the collapsed state gains the whole-bar target. | At touch scale a 50 px sliver with one 44 px button at the end wastes its own target area. |
...
| **G3** | **Three dials, not one**: *targets* ≥ 44 (bar buttons, thumbs, handles), *glyphs* ~24 inset in the target box, *indicators* thin (scrollbars are NOT targets). A widget's hit box may be larger than what it paints (the bar arrange must never equate glyph with box; equal on the desk profile so today's pixels don't move). Amended 2026-08-24 (owner ruling at Plan 3 P0): the PROMPTER family and prompt Ok/Cancel buttons (and slider thumbs) are targets under this rule — no exemption; a confirmation button is the most consequential tap in the interface. | Uniform scaling is what looks silly; the HIG scales three kinds of thing differently. |
| **G4** | **Scrollbars become overlay INDICATORS**: thin, fatten under a hovering pointer. The frame's thickness formulas never add `scrollBarsThickness` (bars overlay the plane — verified in `ViewportWdgt._reLayoutScrollbars`). ⚠ Amended 2026-08-24 (owner ruling at the Plan 3 P2 probe): the indicator is **ALWAYS VISIBLE while content overflows** — the original appear-on-scroll/fade-after-linger behaviour is dropped and its fade clock never exists (visibility is a pure function of overflow; the determinism question dissolves). Cropping-visibility heuristics were rejected as paint reading content semantics. | The single-interface answer; a fat bar is the per-device one; an invisible indicator on an overflowing pane hides scrollability. |
| **G5** | Targets 44; **menu row height is the one dial the owner sets by taste** (44 per HIG; 40 is the floor — Fluent's touch minimum). The cap-to-world + scroll rule (C10) carries long menus either way. RULED 2026-08-24 (Plan 3 P0, pre-probe): **44**; the P2 probe page remains the eyes-on confirmation, where the owner may still re-turn it. With it, ruled the same day: `toolRows` **1** (iPad convention; T2's chevron absorbed into Plan 3 as its pair), and the `_makePrettier` suite-vs-product value fork is FOLDED into the one preference block. P2-probe rulings (2026-08-24): adjacent menu/list rows get a **faint painted hairline separator** — the rows container derives adjacency at arrange (the C5 derived idiom), each row paints an inset hairline when told; paint-only (`menuRowSeparatorColor`), never a layout gap (1px gaps make dead zones between touch targets). Row-label fonts re-proposed at the probe (menuFontSize → 17, titleBar → 15) pending the owner's eyes. | — |
| **G6** | **The visual wave scales the test-world extent with the geometry** so the suite keeps its meaning (at 44 px rows a ten-row menu is the whole 440 px test world). RULED 2026-08-24 (Plan 3 P0): **factor 2 — 960×440 → 1920×880**. The ruling was taken on the plan's 44/~20 ≈ 2.2 estimate; P0's census then measured today's row at **15 px** (strict ratio 44/15 ≈ 2.9 → factor 3 = 9× pixel area), and the coordinator's recommendation — factor 2 stands on suite cost; chrome's world share moves ~39% → ~55%, within taste, every overflow fixture keeps its meaning — was reported to the owner. Re-turnable at zero cost until Plan 3 P4 lands the extent. | — |
...
|---|---|---|---|---|
| T1 | **Command-panel unification** — `MenuRowsPanelWdgt` (column of labels) and `ToolPanelWdgt` (grid of icons) become ONE command panel over a **command model** (a command = icon + label + target/action + selection state), projected along two axes: arrangement `'column' \| 'grid'` × display `'icon' \| 'label' \| 'icon+label'` (the toolbar ⇄ menu axis, C16). EXPANDED 2026-08-24 by the owner at the Plan 3 P2 probe: the display axis is the owner's addition (labels with their icons at the left; a chevron may pop its remainder as a vertical toolbar OR a labeled list OR icon+label rows — the same collection re-projected). The Plan 3 chevron's derived remainder menu (`[icon, string]` rows) is the live proof the projection works. | session | **CLOSED 2026-08-26 (Plan 3.5, `docs/archive/command-panel-unification-plan.md`)**: the MODEL landed — `CommandSpec` (né MenuItemSpec) = the one command record with the `icon` slot and the dispatch-contract law in its header; the display axis live (chevron remainder rows icon+label, rasterize-once inert icons); `CommandPanelWdgt` (né MenuRowsPanelWdgt) the renamed column panel. ⚠ Two row premises corrected in execution, dated in the plan: the `[icon, string]` "live proof" was a COMMENT with no mechanism (P2 built the first real one), and the ONE-CLASS merge was FALSIFIED by measurement (17 friction members) — the pre-authorized fallback taken, owner-ruled: both panel classes stand, the unification is the record + law + axes. Rider landed (owner-directed at OD3): the **`User` model** — the armed drawing tool is the PERSON's fact, palettes are views/controllers on one announce edge each. | **closed** |
| T2 | **Overflow chevron** — a docked strip showing as many items as fit + an overflow button popping the rest as a menu (macOS/iPad toolbar convention; T1 from the other side). ⚠ Live motivation since P5: the grip bar eats strip length, so Docs Maker's last tool now sits behind the strip's scroll. | session | CLOSED 2026-08-25 (Plan 3 wave): `OverflowChevronButtonWdgt` shipped — fit derived by the arrange, remainder menu derived at pop (world child, on top, editor-chrome), no chevron when all fit; witness `SystemTest_macroToolbarChevronPopsOverflow` | **closed** |
| T3 | **Resize handle: small glyph, large hit zone** (or edge-grab resizing). RULED 2026-08-24 (owner, Plan 3 P0, from the three shapes weighed in the plan's §2.5): **(a) big glyph — glyph = box, a 44 px drawn grip**; what you can hit is what you see. No `handleGlyphSize` preference is introduced; edge-grab was not wanted and gets no tail row. | session | CLOSED 2026-08-25 (Plan 3 wave): the 44 grip shipped, hit surface = the drawn TRIANGLE (`HandleAppearance.shapeContainsPoint`), hidden while collapsed | **closed** |
| T4 | **Multi-pointer** (pinch from two `pointerId`s; retire `gesturestart/gesturechange`). | session | FILED 2026-08-27 (Plan 4 close): `docs/BACKLOG.md` row — the Safari `gesture*` no-op listeners stay beside the pointer listeners until pinch is derived from two `pointerId`s, as ruled (I1) | **closed** |
| T5 | **Desktop-edge docking** (the world's edges as dock slots — a taskbar strip). P5 decided 2026-08-23: `EdgeDockLayoutSpec` is world-CAPABLE (names no host type) but the world grows no slots — a desktop dock needs a dock loop in the world's arrange, a fill gesture, and a stated precedence against the `StretchLayoutSpec`s the world already consumes, all unruled. | session | FILED 2026-08-27 (Plan 4 close): `docs/BACKLOG.md` row | **closed** |
| T6 | **Hover-dependent affordances on touch** — toolbar thumbnail tooltips (`GlassBoxTopWdgt.toolTipMessage`), hover highlights, the pointer-under state. | session | CLOSED 2026-08-27 (Plan 4, archive plan §2.4): widget identity on touch = the hold-opened menu's TITLE (the hold fires the right-click's own titled menu; the one untitled menu — the dev-mode hierarchy chooser — owner-ruled stays); tooltips + indicator fattening declared OUT for touch; the pointer-under state DISSOLVES at a touch stroke's end (the leave sweep + the pointer-absence state — nothing is under a pointer that is not there) | **closed** |
| T7 | **Virtual keyboard keyed on the `pointerType` of the tap that started the edit** (today keyed on the session-level `isTouchDevice`). | session | CLOSED 2026-08-27 (Plan 4 P1): both seams (`WorldWdgt.edit`, `_initVirtualKeyboard`) key on `world.hand.pointerType is 'touch'` AND `useVirtualKeyboard`; the never-true `isTouchDevice` RETIRED; witness `SystemTest_macroFingerTapEditSummonsVirtualKeyboard` (assertion-only, teardown vmtruth-judged) | **closed** |
...
| T9 | **Stale comment** in `PromptWdgt` ("the three isMenu? sites … Wallpaper / StringWdgt tick refresh") — there is ONE consumer (`ActivePointerWdgt:660`). | session | CLOSED 2026-08-26 (found closed-but-unmarked at Plan 4 authoring; coordinator spot-checked): the `PromptWdgt` comment names the ONE `isMenu?()` consumer | **closed** |
| T10 | **`FrameWdgt.tight`** — set in the ctor, read by nobody on the frame (only `VerticalStackPanelWdgt` reads `@tight`). | session | CLOSED 2026-08-26 (found closed-but-unmarked at Plan 4 authoring; coordinator spot-checked): `tight` has zero occurrences in `FrameWdgt.coffee` — only `VerticalStackPanelWdgt` declares/reads it | **closed** |
| T11 | **The live input-mode toggle** — the world menu's "touch screen settings" row, `PreferencesAndSettings.toggleInputMode` / `setTouchInputMode` / `setMouseInputMode` / `inputMode` (+ `INPUT_MODE_*`), which rewrites ~10 preferences per device (the per-device redraw G1 forbids). `fg menusweep` covers the row's removal; `check-dead-methods` the verbs. | Plan 1 P0 (F13 falsified) | CLOSED 2026-08-24 (Plan 3 P1): the row + the whole `inputMode` apparatus deleted; declared 14-test recapture; `menusweep`/`check-dead-methods` police the residue | **closed** |
| T12 | **`check-menu-actions` third blind spot** — a STRING action the target class does not define (the `"pin"` row, dead since 2018 — found by Plan 1 P1). Extend RULE 1: when the target is `@`, resolve the string against the enclosing class + its `extends` chain (+ mixins) and fail on a miss; record the blind spot in `lint-and-static-checks.md` either way. The runtime twin (a `MenuWdgt`/`PromptWdgt` root in `menusweep`) lands with the P1 fix. | Plan 1 P1 | CLOSED 2026-08-24: `check-menu-actions` RULE 1b — an `@`-targeted string action must resolve on the union of every chain containing the enclosing class (subclasses + mixin consumers + harness `installOnto` methods); plant-proven; +0.3 s. Bonus: `coffee-method-header.js` demanded exactly one space after the colon — 9 methods invisible to all 7 gates, fixed (`: +`), no verdict changed. | **closed** |
| T13 | **Infinite recursion on deleting a pop-up attached into a reference/shortcut icon** — `MenuWdgt`/`PromptWdgt` reparented (via "attach…") under `FloppyDiskIconWdgt → DocumentShortcutWdgt → GenericShortcutIconWdgt → GenericObjectIconWdgt → WidgetIconWdgt`, then "delete": `_destroyNoSettle → … → DocumentShortcutWdgt._severReferenceEdgeToNoSettle → …_fullDestroyNoSettle → WidgetIconWdgt._fullDestroyNoSettle → MenuWdgt._fullDestroyNoSettle → _destroyNoSettle` repeats (`Maximum call stack size exceeded`; top frame `noteStructureChange`). Found by the `MenuWdgt[own]`/`PromptWdgt[own]` `menusweep` roots added with the P1 "pin"-row fix; reproduces in isolation (`Fizzygum-tests/.scratch/t12-delete-stack.js`). Those roots keep the gate RED until it is fixed. | Plan 1 P1 follow-on | FIXED 2026-08-23 (Opus `investigate-bug` worker): the sever block of `Widget._destroyNoSettle` runs LAST — after mark/detach/empty — so the containment edge into the dying widget is gone before the first sever; regression test `SystemTest_macroDeletingMenuNestedInShortcutIconTerminates` (assertion-only, plant-proven); `reference-widgets-plan.md` §4.3's termination argument corrected | **closed** |
| T14 | **`FrameBarWdgt._destroyEditButtonNoSettle` retires an icon button with plain `_destroyNoSettle`** — the button owns a `face: new Widget`, and retiring the close piece the same way in P2 left 88 `WORLD_INVENTORY_ESCAPED/DRIFT` corpses (fixed there with `_fullDestroyNoSettle`). The edit piece is only retired when a frame LOSES `providesAmenitiesForEditing`, which no test does today, so the gate is green for want of a witness. | Plan 1 P2 | CLOSED 2026-08-26 (found closed-but-unmarked at Plan 4 authoring; coordinator spot-checked): `_destroyEditButtonNoSettle` retires via `@editButton?._fullDestroyNoSettle()`, and the witness `SystemTest_macroRetiredBarPieceTakesItsFaceWithIt` exists | **closed** |
| T15 | **`closeFromFrameBar` sits in `buildSystem/public-api-allowlist.txt`** because rule [U] reads it as self-only once no test prose names it — it is the bar's close verb on the frame. Either the bar calls a `_`-tier twin (churns inspector member lists = pixels) or the allowlist entry stays with its reason. | Plan 1 P2 | CLOSED 2026-08-26 as option (b) (found resolved at Plan 4 authoring; coordinator spot-checked): the allowlist entry STANDS with its stated reason — "the bar-press handler is its only in-tree caller (same shape as setScrollPolicy above)" | **closed** |
| T16 | **`@serializationTransients` on a subclass REPLACES the inherited list** — the meta-system copies class statics down, so a subclass declaring its own list silently drops `Widget`'s 31 entries (the serializer's merge comment claims "a subclass ADDS"). Caught by the rig when `FrameWdgt` declared one in P3; fixed there by declaring the name on `Widget`. Nine classes carry their own declarations today (grep 2026-08-24): `ScriptWdgt`, `TransformFrameWdgt`, `CellWdgt`, `SheetHeaderCellWdgt`, `SheetCellRecord`, `SimpleSpreadsheetWdgt`, `DesktopAppearance`, `CalculatingPatchNodeWdgt`, `FridgeMagnets3DCanvasWdgt`; the rule is stated in `serialization-duplication-reference.md` §5. | Plan 1 P3 | CLOSED 2026-08-24: the serializer MERGES the chain (choice (a); no class relied on replacement — `TransformFrameWdgt`'s comment even asserted the merge). Root cause: the walk used `Object.getPrototypeOf` but `extend` links via `__super__` only, so it stopped at the first class; now walks `__super__`. `isPopUpMarkedForClosure` back on `FrameWdgt`. Rig check plant-proven (`transients.inheritedMergedNotReplaced`). Doc §5 states the merge. | **closed** |
| T18 | **Halo crowding at touch scale** — at `handleSize 44` the five corner-internal halo handles fully overlap on any widget smaller than ~2·handleSize (~92 px); Plan 3 found the rotate handle (whole-box hit shape) shadowing the corner resizer's triangle entirely on a 50×40 subject, turning resize presses into rotations. Plan 3's interim fix is z-ORDER (the corner resizer — the only handle whose reactive shape is a proper subset of its box — is frontmost), but five stacked 44 px handles on a tiny widget is an unanswered DESIGN question (fewer handles below a size floor? edge-zones? a halo mode?). Related: the owner-pending G2 halo feel-check. | Plan 3 P3 (found at the wave gate) | FILED 2026-08-27 per OD4, owner PRE-RULED (a) at the Plan 4 review: `docs/BACKLOG.md` row, PAIRED with the owner-pending G2 halo feel-check (one conversation — how the halo should FEEL at 44 px decides the T18 design). Plan 4's grammar did not restructure the halo: handles are chrome (`ownsDragsStartingOnMe`, no hold), the Plan 3 z-order interim stands | **closed** |
| T17 | **`SwitchButtonWdgt.mouseClickLeft` escalates with ITSELF where every other sender passes the POSITION** — its own documented pattern break; it silently defeated P5's first C17 guard (8 collapse tests double-toggled). The C17 guard now accepts both shapes; the clean fix is the switch forwarding the position (a shared-class change). | Plan 1 P5 | CLOSED 2026-08-24: the switch forwards the position; the C17 guard reduced to the position shape; all 23 `escalateEvent` sites reviewed — no other self-passing escalator (`ButtonWdgt`'s `arg1` is positionally the pos, a naming nit). | **closed** |
...
|---|---|---|---|
| 1 | `docs/archive/frame-lifetime-and-docking-plan.md` (CLOSED) | 2026-08-23 | — |
| 2 | `docs/archive/pointer-events-plan.md` (CLOSED) | when Plan 1 closes | Plan 1 edits the hand's dismissal paths; a plan authored now would cite lines Plan 1 moves. |
| 3 | `docs/archive/single-geometry-visual-wave-plan.md` (CLOSED) | when Plan 2 closes | The bar/dock metrics it retunes are Plan 1's output. |
| 3.5 | `docs/archive/command-panel-unification-plan.md` (T1 as expanded; CLOSED 2026-08-26) | when Plan 3 closes | Its subject classes are Plan 3's output (chevron, dials); landing pre-Plan-4 keeps finger references off classes it churns. |
| 4 | `docs/archive/gesture-grammar-and-finger-harness-plan.md` (CLOSED 2026-08-27 — the LAST plan; its STATUS box is the as-executed log) | when Plan 3.5 closes | Cites the `PointerInputEvent` family (Plan 2) and captures at final geometry (Plans 3/3.5). |
