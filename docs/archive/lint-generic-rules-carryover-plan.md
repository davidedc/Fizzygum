> **ARCHIVED — COMPLETE (2026-07-17 restructure).** EXECUTED 2026-07-15, Phases 1-6 complete; keeps the record + §8 backlog.
...
`docs/architecture/lint-and-static-checks.md` (gates §3, the severity policy §3b, the censuses §3c); the findings
...
`/Users/davidedellacasa/code/Fizzygum-all/Fizzygum/docs/architecture/lint-and-static-checks.md` in full before
...
- New §3-table row + §5-allowlist row added to `docs/architecture/lint-and-static-checks.md` (see Phase 5).
...
- **Deliberate seams exist and must not be "cleaned up":** the duplication-refactor arc
  ...
  in `docs/tooling/duplicated-code-detection.md`) established that the `_apply*/_commit*` corner twins, the
...
- Findings are APPENDED (as a new dated section) to the duplication triage ledger
  ...
  section, noting the convention source `docs/tooling/duplicated-code-detection.md`).
...
7. Footer: "advisory only — nothing here gates; see docs/archive/lint-generic-rules-carryover-plan.md".
...
4. ONE end-of-arc review pass over the whole diff (`git -C /Users/davidedellacasa/code/Fizzygum-all/Fizzygum diff` +
   ...
   `build_it_please.sh`, `docs/architecture/lint-and-static-checks.md`, this plan's STATUS line; NOTHING under
...
3. **A4 — paired-method contracts.** Table-driven "defines X ⇒ must define Y" (Pharo:
   ...
   runtime cache-oracle gate would catch it). Survey `docs/architecture/serialization-duplication-reference.md`
...
5. **C — metrics ratchet.** Per-file line/method-count baselines for the top-N god files
   (`Widget.coffee` = 4,950 lines) to support `docs/archive/god-class-decomposition-plan.md` ("must not
