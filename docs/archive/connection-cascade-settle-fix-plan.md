> **ARCHIVED — COMPLETE (2026-07-17 restructure).** Shipped (commit 14014e44); _setTextConnector / _fireConnection / layering rule [P] all confirmed live in source.
...
- **Static gate:** `buildSystem/check-layering.js` — its rule `[O]` (`COALESCED_CALLER_ALLOWLIST`) is the template for
  the new `[P]` (§3 step 5); reference docs: `docs/architecture/lint-and-static-checks.md` §4 + `docs/architecture/layering-naming-convention.md`
...
- **Behavioural gate:** `./fg gauntlet` (dpr1/dpr2/webkit + apps smoke + audit gates) + the four-config torture (§5b),
  plus the §5a acceptance probe. The new settle-lane is documented in `docs/archive/layout-system-architecture-assessment.md`
  §2.2 and `docs/architecture/layering-naming-convention.md` §2.5.
...
`git -C Fizzygum status --short docs/`): `docs/architecture/lint-and-static-checks.md` (~:177),
`docs/architecture/layering-naming-convention.md` (§2.5 ~:113-123 + §4 ~:225), `docs/archive/layout-system-architecture-assessment.md`
