> **ARCHIVED — COMPLETE (2026-07-17 restructure).** HISTORICAL — Phase-1 design-of-record, shipped (ad2000cc/b8165920); canonical entry point moved to the OVERVIEW doc.
...
- **Phase 3b — Slice 2 (✅ SHIPPED 2026-06-20 — `6c7060e5` (_reLayout) + the flow-rule app-freeze fix `c45113ac` + lint `b89c9141`). CURRENT SELF-CONTAINED RECORD + NEXT STEPS: `docs/archive/deferred-layout-slice2-completion-plan.md` — read that; the bullets below are historical (the "context-aware rawSetWidthSizeHeightAccordingly" they describe was SUPERSEDED by the flow rule).** flip the ~20 inline `_reLayoutChildren` content-change
  ...
  - **⚠️ SUPERSEDED HISTORY (2026-06-19) — what follows describes the FIRST fix attempt (a context-aware
    ...
    schedule layout), `c45113ac` + `b89c9141` — see `docs/archive/deferred-layout-slice2-completion-plan.md` §2. Kept below
