> **ARCHIVED — COMPLETE (2026-07-17 restructure).** Phases 0-5 clean wins DONE 2026-06-17 (complete per project ledger); Phases 6-8 folded into god-class-decomposition-plan
...
2. **Risk is dominated by two things in this codebase:**
   ...
   - *Does the change touch the rendering loop, `_reLayout`, or `ActivePointerWdgt` input
...
any `STATE_PRESSED`-based selection — override to a no-op where that matters; (3) `ButtonWdgt._reLayout`
lays out a `faceMorph` — a button without one must call `Widget::_reLayout.call @, …` directly (the 6
...
**⤳ SUPERSEDED by `docs/archive/type-test-elimination-plan.md`** (the codebase-wide umbrella; absorbs the
...
- **6b — `Widget` (4728 L → thinner):** peel cohesive clusters into mixins/collaborators in
  ...
  (`spawnInspector`, `uniqueID*`), animation — then last the **`_reLayout` (`:4216`, 99 L) / geometry**
...
**Volume:** the largest by far. **Risk:** HIGH (broad; `_reLayout` is determinism-sensitive). **[DET]:**
...
self-contained execution plan now exists: `docs/archive/god-class-decomposition-plan.md`** (study findings per
...
- **7c — `SimplePlainTextWdgt`** grandparent writes → `setTextLineWrapping(bool)` on the
  ...
  **Path-B** case (it entails a constrained re-wrap, so `@desired*` ≠ the constrained result) **plus** an
  ...
  to the scroll panel, and the wrap geometry in `adjustContentsBounds` is off the `_reLayout` path — wiring
  ...
  treatment: **`docs/archive/softwrap-deferred-layout-conversion-plan.md`**. (This assessment also deleted the dead
...
(`setExtent`/`fullMoveTo` → `invalidateLayout()`, derived by `recalculateLayouts`→`_reLayout`). **~430 sites
...
intermediate). Full write-up + the A/B path catalog: **`docs/archive/softwrap-deferred-layout-conversion-plan.md`**.
...
**`docs/archive/deferred-layout-path-a-design.md`** (supersedes the "Path A = pending-aware accessors" framing here).
...
- **Path B — per-site** (eliminate the read-back / re-order the constraint): when the event hits a constraint,
  ...
  **sideways** drag, stack elements, ratio locks, clamps, `FIT_BOX_TO_TEXT` wrap. A pending-aware read would
  ...
  fix (its wrap geometry in `adjustContentsBounds` is off the `_reLayout` path; wiring it in flips
