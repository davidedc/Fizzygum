...
- `Fizzygum-tests/scripts/check-tests-syntax.js`, `Fizzygum-tests/scripts/check-refs.js`,
  `Fizzygum-tests/scripts/check-macro-eval-discipline.js` — the last one's rule is a pure function
  (`violationsInTest`) so its acceptance corpus, `check-macro-eval-discipline-selftest.js` in
