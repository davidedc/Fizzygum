...
| **menu-actions** | **`buildSystem/check-menu-actions.js`** | **~:509** | **menu-item wiring. A menu action is dispatched through ButtonWdgt's fixed four-slot convention (`@target[@action].call @target, menuItem, panelTarget, arg1, arg2`), which the call site never names. HARD (sound): a function literal in the action slot (`@target[@action]` coerces it to a string key, so the click throws); a string literal where `opts` goes; and — RULE 1b — a `@`-targeted string action that RESOLVES nowhere. RATCHET at 0: an UNREAD parameter on a menu-dispatched verb must be NAMED as unread (`ignored`/`unused`) — padding a signature to reach the slot you want puts widgets into parameters whose names promise otherwise, and forces every other caller to punch `undefined` through.** ⚠ TWO blind spots, both structural: it cannot see whether a parameter that IS read is read as the right THING; and it cannot see a parameter that is MISSING — a verb needing a subject it never declares reads as clean, because there is nothing unread to flag. (A THIRD is covered by RULE 1b: an action naming a method NOTHING defines. The general case stays unresolvable — the receiver of `addMenuItem "…", someExpression, "verb"` is a runtime value — but `@` names its receiver structurally, so the rule resolves the string against the union of `chainOf(X)` over every X whose chain contains the enclosing class, which is exactly the set `@` could be an instance of, plus the methods a `*TestSupport` class installs onto a core class. That union is what makes it SOUND: `Widget` wires `"setTargetAndActionWithOnesPickedFromMenu"`, which lives in `ControllerMixin`. 136 rows covered; the dead `"pin"` row of 2018 is the shape it catches. The runtime twin still owns the rest: non-`@` targets, and the dispatch itself.) Those blind spots are the class of bug only a rig that CLICKS the item can catch, which is why `fg menusweep` exists beside it — it resolves every row's action against its target BEFORE the liveness skip (so a row sitting behind a destructive one in the same walk is resolution-checked even where it is never dispatched) and carries a `MenuWdgt` and a `PromptWdgt` root, the two classes whose own context menu no other root stands in. ⓘ RULE 1 has TWO DOORS: besides `addMenuItem`/`prependMenuItem`, `prompt`/`textPrompt` take a `callback` that `PromptWdgt` hands to a menu item verbatim (`panel.addMenuItem "Ok", @target, @callback`) — same slot one hop later, so the same proof applies, and those callbacks count as menu-dispatched verbs for the RULE 3 ratchet too | rules 1–2 are HARD; the unread-parameter rule is the inline `RULE3_BASELINE = 0` ratchet |
...
| raw-pointer-reads | `buildSystem/check-raw-pointer-reads.js` | ~:731 | a pointer-event HANDLER body (`press*`/`hover*`/`activated`/`scrolledBy`/`nonFloatDragging`/…, closures included) must not consume the raw SCREEN-plane `world.hand.position()` unmapped — consume the plane-mapped `pos` PARAMETER the dispatcher hands every handler (affine 4A), or map the sample at the read site (`screenPointToMyPlane` on the same line, the drag-scroll idiom). Off-island the mapped point IS the raw point, so the bug class is invisible until a widget is TILTED (the 2026-07-17 spreadsheet tilted-selection bug). Helpers a handler calls are NOT scanned (heuristic tripwire — the deliberate raw-screen rotate-angle helper lives in one); `ActivePointerWdgt` itself skipped | `# raw-screen-pointer-sanctioned: <reason>` above the handler header (currently ZERO sites) |
| plane-discipline | `buildSystem/check-plane-discipline.js` | ~:740 | the three statically-checkable rules of the paint-time scroll model ([`viewports-and-planes.md`](viewports-and-planes.md)): **A)** `scrollOffsetX/Y` assigned ONLY inside `_writeScrollOffset` (a bare write skips the `geometryVersion` bump — the measured hit-invisible-scrolled-row class); **B)** a count RATCHET over lines mixing two DISTINCT receivers' POSITIONAL geometry (`position`/`left`/`top`/`center`/`bounds`/… — plane-relative, unlike the plane-invariant extents) with no mapping call on the line — the silent dormant-at-offset-0/identity class every hard bug of the scroll arc belonged to; same-plane-by-construction sites are legitimate, so new lines are CLASSIFIED (map, or raise the baseline with a reason) and drops ratchet down; **C)** a positional pointer handler on a `scrollTranslationOfChild` provider (transitive via `extends`) must re-derive its `pos` through `screenPointToMyPlane` — `escalateEvent` forwards descendant-plane args verbatim (the measured stationary-click-slams-to-clamp defect; `wheel` exempt, deltas). ⚠ Blind by design to CROSS-LINE flows: a positional value stored in one method and consumed in another plane elsewhere, or forwarded through an opts key — those are the deep-audit's job (the 2026-08-20 review census), not a line scanner's | A/C: hard zero; B: inline `CROSS_PLANE_MIX_BASELINE` (35 — every line a genuine two-widget mix; a widget's own `@bounds` chain folds into `@`); C: `# escalated-pos-sanctioned: <reason>` above the handler header (currently ZERO sites) |
| test-.js syntax | `Fizzygum-tests/scripts/check-tests-syntax.js` | ~:770 | JS syntax of the macro SystemTest `.js` files the build is about to serve | — (self-skips unless `$PROFILE_SHIPS_TESTS`, or when the sibling repo is absent) |
| macro source discipline | `Fizzygum-tests/scripts/check-macro-source-discipline.js` | ~:794 | three rules over MACRO SOURCES only (the backtick spans of `*_automationCommands.js`, `#` comments stripped): **A)** `@evaluateString` is ALWAYS wrong — in macro source `@` is the `MacroToolkit`, whose `evaluateString` is a DIFFERENT method (binds `@` to the toolkit, omits the repaint tail); it compiles fine and misbehaves at run time. **B)** `world.evaluateString` only where the test DECLARES the eval as its subject, via the `evaluateString` tag in its metadata — because a macro body IS CoffeeScript, so the string form throws away the widget binding, the syntax gate and any rename sweep's chance of finding it, AND `Widget.evaluateString` ends in `_reLayoutSelf()` + `_changed()`, so the world form damages the WHOLE world: a fixture step written that way forces a full repaint that MASKS a missing self-invalidation in whatever it just drove — the exact class the incremental damage-rect screenshots exist to catch. ⭐ A DECLARATION, not a count: there is no baseline number to rot, a legitimate new eval-subject test just tags itself, and the tag is reviewable in the diff (the `storage-invariant-gate.js` no-baseline pattern). **C)** "Macro" only as a TRAILING suffix — `Macro._replaceMacroInvocationWithYieldingInvocations` rewrites every `…Macro` not followed by `(` into a `yield from`, so a mid-name one (`recordMacroAssertion`) is silently MANGLED; that is why the assertion sink is reachable only from a toolkit `@assert…` method. ⛔ A FOURTH rule was measured and DECLINED — "`@x` in macro source must be a MacroToolkit member", which `src/macros/CLAUDE.md` calls the `@`-vs-`world.` "#1 trap": the census found ZERO real violations and exactly one hit that was the CHECKER's own false positive (`box.hoverMoved = (pos) -> @recordedHoverMovedPos = pos` is correct — `->` REBINDS `@`), and checking it properly needs `@`-rebinding analysis through `->` vs `=>` that a line scanner cannot do. ⚠ Scans macro sources ONLY, and that precision is load-bearing — metadata prose and macro comments legitimately discuss `world.evaluateString`, so a whole-file grep would fire on tests doing exactly the right thing | — (self-skips like the test gate). Its own acceptance corpus is `check-macro-source-discipline-selftest.js` in `npm run selftest`, whose ship gate is the NEGATIVE cases plus a CONTROL differing from a caught case only by the tag |
| ref-image integrity | `Fizzygum-tests/scripts/check-refs.js` | ~:817 | >1 `dataHash` per `(test,image,dpr,OS)` or an orphaned `.js`/`.png` reference | — (self-skips like the test gate). Its PIXEL half, `--pixels`, is deliberately NOT on the build: it runs as the gauntlet's `refs` leg |
| visualisation pages | `Fizzygum-tests/scripts/check-visualisations.js` | ~:836 | **(1)** every local `<img src>` in every committed `visualisation.html` resolves to a file that exists, and **(2)** a test that HAS reference images HAS a page at all — (2) exists because an ABSENT page contains no broken image, so (1) structurally cannot see it (the 2026-08-21 sweep found 3 tests with no page whatsoever). ⚠ (2) keys on HAVING REFERENCES, not on being a test: a test dir mid-authoring legitimately has neither yet and its page is written by the first capture, so requiring a page of every test would fail the build on a half-authored one — a false positive, not a find. Assertion-only tests are exempt by that rule and have pages anyway. ⭐ The break it catches is SILENT and therefore unbounded in time: a page EMBEDS its reference filenames, a reference filename carries the image's `dataHash`, and **nothing in CI ever opens these pages** — so a page orphaned by a rename or a recapture is indistinguishable from a working one until a human clicks it. Measured twice: 10 of 305 pages were already broken on 2026-08-21, and the 2026-08-22 reference filename-grammar migration would have broken 279 in a single commit. The capture path regenerates a page at its write site (`run-macro-test-headless.js`), which covers captures; this gate covers **everything else** — bulk renames, hand edits, a deleted reference — by checking the property itself instead of trusting every writer to remember the rule. No image decode, no browser. ⚠ It checks RESOLUTION, not freshness: a page whose `<img>`s all resolve can still show the wrong step order or stale prose | — (self-skips like the test gate). Fix with `node scripts/make-visualisation.js <name>` (no arg = every test). Its `--strict` mode also requires each page to EQUAL what the generator produces now — freshness, not just resolution — and is deliberately NOT on the build: that residue is cosmetic, while a page-TEMPLATE edit would otherwise break every build until all pages are regenerated (`npm run check-visualisations:strict`). Acceptance corpus: `check-visualisations-selftest.js` in `npm run selftest` |
...
A header is either **closed on the line** (`foo: ->`, `foo: (a, b) ->`, `foo: (a)->`, `foo: (a) =>`,
`foo:  ->` with the body aligned past a run of spaces) or **opens a wrapped parameter list** (`foo: (`
with nothing after the paren; the continuation lines are ordinary body lines to every gate, which is
correct — they are the signature).
...
⚠ **It happened a THIRD time, 2026-08-24**, found while building RULE 1b of `check-menu-actions` (which
must know whether a class defines a method at all): the matcher demanded EXACTLY ONE space after the
colon, so nine methods whose author aligned a short body — `softWrapOn:  -> @setSoftWrap true`, the six
`buildWindow:  -> …` app doors, `DataflowEngine.frameSource`, `DoubleLinkedList.constructor` — were
checked by no gate. The guard could not see them either: its own precondition demanded `: \S`, and a
second space is not `\S`. Both now read `: +`. The lesson each time is the same one, and it is about
where the matcher's authority comes from: write it against the LANGUAGE, never against a house
formatting habit. Widening it moved the method total 3305 → 3314 and changed no gate's verdict.
...
- **`escalateEvent`'s plane-local arguments have NO gate — the discipline is a stated rule, enforced by review.**
  Base `Widget.pressBegan`/`activated` escalate their `pos` up the parent chain verbatim, so a pos that
  ...
  world.hand.position()`, as `ViewportWdgt.pressBegan` does — the paint-time-scroll arc's case law: a
...
- `Fizzygum-tests/scripts/check-tests-syntax.js`, `Fizzygum-tests/scripts/check-refs.js`,
  `Fizzygum-tests/scripts/check-visualisations.js`,
  `Fizzygum-tests/scripts/check-macro-source-discipline.js` — the last one's rule is a pure function
  (`violationsInTest`) so its acceptance corpus, `check-macro-source-discipline-selftest.js` in
  `npm run selftest`, can plant a violation in memory and require the gate to catch it.
- `build_it_please.sh` — gate wiring (~:337–845), each `if ! $noSyntaxCheck` + `$?`-gated `exit 1`; the four
  tests-repo gates additionally `&& $PROFILE_SHIPS_TESTS && [ -d ../Fizzygum-tests ]`.
