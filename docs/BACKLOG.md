...
- [ ] P10(b): index button `@target`s as non-traversed COMMAND edges — rides `plans/graph-edges-and-lifecycle-plan.md`, not this arc. ⇒ **Absorbed 2026-08-18: that plan (revised to executable shape the same day, every current-state claim re-verified) now carries it as its §4.2(b) / decision G4**, with a recommendation of LAZY enumeration via the `graphEdgesOut` protocol rather than an eager index (no `@target` write funnel exists, menus would churn the index, no reverse-query consumer). Owner decisions G1–G8 there are pending; work it when that plan is worked.
