...
- [ ] **Suite nondeterminism — 2 OPEN non-boot flakes** `plans/suite-nondeterminism-flakes-plan.md` (2026-07-28). Flake A `SystemTest_macroClosingRotatedIslandChildClearsFootprint` (assertion-only, proven to fail on unmodified master, class-3a irregular-load race); flake B serialization-rig `pixelParity` BISTABILITY (two converged renders; leading hypothesis = lazily-loaded glyph atlases + the rig never gating on `anyTextDirty()`). Start with flake B — it has a one-line candidate fix and reproduces ~1-in-3 gauntlets.
...
- [ ] `plans/resetworld-teardown-completeness-audit-plan.md`: audit `WorldWdgt._resetWorldNoSettle` for completeness — enumerate every piece of world-level mutable state and decide each (reset / deliberately-survives / N-A), then add a structural guard so a newly added field cannot silently reopen the hole. PLAN ONLY, not started. Prompted by two field-caught leaks of the same shape (`widgetsToBeHighlighted` 2026-07-10; `UntitledNamingService` counters 2026-07-28, DETERMINISM.md §3h). ⛔ an s1 gauntlet leg is owner-DECLINED — `--shards=1` is an audit tool here, not a new gate.
