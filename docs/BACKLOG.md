...
- [ ] §5.D: D-1 paint-on-focus — execution design + S1 spike ✅ 2026-07-20 (spike: focus premise / rebind byte-safety / tilted-island stroke ALL PASS, evidence in plan §5.D D-iii); D1b landing = `ImageWdgt` + `PaintToolbarWdgt` + press-time `paintingOverlay()` resolution, `ReconfigurablePaintWdgt` + `StretchableEditableWdgt` DELETED; REMAINING: D-2 one editing-focus model (caret + tool-head + focus pointer + `StringWdgt.selection`) — its OWN design pass, gates on `Fizzygum-tests/DETERMINISM.md`
