...
- [ ] `archive/build-arc-4-dynamic-parts-plan.md` §5.4 (banked): move the four `LayoutElementAdderOrDropletWdgt`-touching `Widget` members (`showAdders`, `addOrRemoveAdders`, `_insertAddersSuchThat` + kin) out of core onto the `dev-tools` part as a `…Support.installOnto Widget` extension class — arc 3's `WidgetTestSupport` mechanism reused for a part rather than the harness. Arc 4 phase 1 only GUARDED them (`return unless LayoutElementAdderOrDropletWdgt?`), which is correct but leaves four members in core that a production build can never use.
