...
- [x] §5.D D-3: VISIBLE editor-focus indicator — ✅ BUILT + GAUNTLET GREEN 2026-07-20 (plan §5.D D-3-v; commit pending owner approval). `WorldWdgt.addEditorFocusIndicatorWidget` reconciler (rides the `addHighlightingWidgets` declare-and-reconcile) draws ONE `HighlighterWdgt` teal OUTLINE (D19, `Color 38,166,154`) around `_widgetBeingEdited()` — `editorFocusWdgt` WHEN a caret targets it (text) OR its `providesAmenitiesForEditing`+`dragsDropsAndEditingEnabled` (citizen), with a load-bearing world-guard (a desktop click focuses the world, itself a PanelWdgt with both flags true). ⚖ OWNER RULING: KEEP BOTH branches (D18 as ratified) — the citizen branch frames any focused/dropped editable panel because the pencil flag defaults true; the caret-only alternative was offered + declined (do-not-re-litigate). Footprint: 44 existing tests recaptured + guard test `SystemTest_macroEditorFocusIndicatorTracksEditedWidget` (frame alpha → retarget bravo → gone). D-2 re-open question RESOLVED: the indicator consumes only the focus pointer, so the four-way `FocusWdgt` stays CLOSED. `world.keyboardEventsReceivers`' three member kinds (caret / `SimpleSpreadsheetWdgt` / `VideoPlayerWdgt`) still unify only with their own consumer
