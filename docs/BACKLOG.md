...
- [ ] `plans/teardown-shared-core-plan.md`: give `_resetWorldNoSettle` (test, homepage-stripped) and `_teardownForSnapshotLoadNoSettle` (product) ONE shared structural core, so the dangling-ref half cannot drift again (it has drifted twice in two days, in both directions). PLAN ONLY, authored 2026-07-29, not started. Fixes two measured PRODUCT leaks on the snapshot path: the `infoDocFlags` restore is additive-only (a flag set before a load survives it forever, so that info doc can never be created again) and `trackChanges` is not serialized at all (an unbalanced stack stops the world recording damage). ⚠ RUN BEFORE build arc 3 — every row that becomes shipping core is a row arc 3's verbatim relocation no longer has to move, and arc 3's inventory line for this code is stale (`~66 l` recorded vs **302** actual).
- [ ] `archive/resetworld-teardown-completeness-audit-plan.md` §7.5 "DELIBERATELY NOT DONE" — SUPERSEDED by `plans/teardown-shared-core-plan.md` above, which subsumes it. Original note: the PRODUCT-side twin `_teardownForSnapshotLoadNoSettle` has the same dangling-ref gaps (`errorConsole` — silently swallows every later paint error in the session; `wdgtsWithOngoingScrollMomentum`; the tooltip/pop-up/handle sets; `lastEditedText`). Checked and NOT a gap there: `numberOfIconsOnDesktop` round-trips (prototype `0`, and `0?` is true, so it is always written and always restored). Evidenced but NOT changed: it ships in `--homepage`, so it is a product-behaviour change wanting owner sign-off + a serialization round-trip verification, not a ride-along on a test-infrastructure audit.
