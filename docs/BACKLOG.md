...
- [x] **Arc C AUTHORED + EXECUTED 2026-08-20/21** (`plans/world-reset-by-reconstruction-plan.md`, STATUS box =
...
- [ ] `plans/world-reset-by-reconstruction-plan.md` §D4b tail — `AutomatorPlayer.createImageFromImageData` (`Fizzygum-tests`) never releases its `img.onload`/`onerror`. It is the same UNRELEASED ONE-SHOT HANDLER family as the three `AutomatorLoader` sites that pinned a dissolved world, but this one is benign on all three counts that made those dangerous: the `Image` never enters the DOM, its `src` is a `data:` URL, and the closure reaches no widget and no world — and the path runs only from `saveFailedScreenshots`, never during a suite. Tidy-up for symmetry with the fixed sites, not a retention fix.
- [ ] `plans/world-reset-by-reconstruction-plan.md` A7 — **six drag-affordance world slots violate the teardown core's stated contract.** `WorldWdgt` declares `dragEmbedChargeRingDeclared` / `dragEmbedLabelDeclared` / `dragEmbedLockBadgeDeclared` (each holding a `{target: <widget>}` record) and `dragEmbedChargeRingWdgt` / `dragEmbedLabelWdgt` / `dragEmbedLockBadgeWdgt` (each an `@add`ed tree child). `_teardownWorldStructureNoSettle` touches NONE of them, so after `fullDestroyChildren()` all six name destroyed widgets — against the core's own one-line contract ("the world holds no reference to anything just destroyed"). ⭐ Arc C MOVED where this bites rather than fixing it: on the `resetWorld` path it is now harmless (the world holding the dangling refs is itself discarded), but `loadWorldSnapshot` shares the core and KEEPS its world, so there the next reconcile pass (`WorldWdgt` ~:1620) calls `updateChargeDeclaration` on — or `fullDestroy()`s — an already-destroyed widget. ⚠ `world.teardownHygiene.*` in `serialization-roundtrip-headless.js` is the gate that should have caught this and does not cover these fields. Fix shape: clear all six in the core, beside the tooltip/pop-up/handle/momentum/highlight collections that are already there; add them to `teardownHygiene`.
- [ ] `plans/world-reset-by-reconstruction-plan.md` §D5c(b) (OWNER DECISION) — **delete two DEAD audit flags.** `WorldWdgt.auditTierAndApplyNaming` and `WorldWdgt.auditNotificationSettleNeutrality` are declared as prototype defaults, armed by their preludes, and READ BY NOTHING (grep-verified across `src` + harness src + `scripts/` + `tests/`: declaration, arming line, a log line echoing the value, a shell header comment, and `_worldStateAuditExemptions` — no branch anywhere). Both gates take every observation from their prelude's own prototype wrappers instead, so arming is a no-op in both directions and the two shell headers claiming the gate "flips on `WorldWdgt.<flag>`" are false. Deletion = 2 declarations + 2 comments + 2 arming lines + 2 exemption entries + 2 shell headers, and it IS the exemption-list pruning §6 lists as an end-of-arc call. ⚠ `auditPaintTimeLayoutScheduling` is a different case — live product code (`Widget._invalidateLayout`), but its own gate re-implements the predicate rather than reading it, so it stays.
- [ ] `plans/world-reset-by-reconstruction-plan.md` §D5c(a) (OWNER DECISION) — **the capstone gate needs a positive-coverage assertion.** Measured 2026-08-21: with `auditUndeclaredEndOfCycle` lost on reconstructed worlds, 307 genuine careless pushes produced `careless pushes=0` and `✓ CAPSTONE GATE PASSED`, with its coverage line reading a full `prelude installed=307/307` — because that number is a prelude-INSTALL count computed from a synthetic header string `run-all-headless.js` writes itself, and is blind to the flag by construction. Fix shape: have the prelude assert per test that the CURRENT world still carries the flag and emit a distinct line, and have `run-capstone-gate.sh` require that count to equal the test count — symmetric with `run-paint-audit.js`'s `checked == expectedTotal`, the one leg besides `fg vmtruth` that can currently fail for having measured nothing. `tiernaming` / `settle` / `storage` / `revisits` share the blind spot (no non-zero-expectation assertion in any of them).
- [ ] `plans/world-reset-by-reconstruction-plan.md` §D4b tail — `swCanvasPoisonedKeyRecorderInstalledOnCache` (`src/boot/extensions/SWCanvasElement-extensions.coffee`) holds ONE dead world's `cacheForImmutableBackBuffers` (up to 1000 canvas pairs) across resets, released only when the next cold-glyph draw installs the recorder on a different cache — which on a warm-atlas page can be many worlds later. Not a WIDGET retention (the cache has no owner back-pointer and no call site passes an `onDispose` closure, so `fg vmtruth` correctly says nothing), but it is real memory no gate covers and it lands in heap-floor readings as noise. The identity-latch design that holds the cache object rather than a boolean is deliberate and documented at its declaration — any fix must keep that property.
