...
- [ ] `archive/mixin-editing-v2-plan.md` "Residual discovered during T": donor attribution covers mixin METHODS only — a mixin-donated FIELD shows its plain value un-attributed (no donor label/override button; save = plain class edit). Field parity needs design: class-inspector field SOURCES vs object-inspector VALUES, `<name>_source` for non-functions, field records in the class-scope registry.
