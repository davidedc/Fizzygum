...
- [ ] `plans/boot-cost-reduction-plan.md` §2.2c-post item 1 — **no gate covers PART→PART edges, and the hole has already shipped four broken doors** (fixed 2026-07-31: `dev-tools`→`demos` for `BouncerWdgt`/`PenWdgt`, `demos`→`maps` for the two map icons; each threw `<Class> is not defined` on a real menu click on `index.html`). `check-part-edges.js` scans CORE only, on the stated premise that part-to-part references are "legitimate when the manifest declares the dependency" — a premise that was never true, since no `requires` field exists. 18 such edges today, 11 into a LAZY part, every one held up by a comment at whichever door remembers. ⚠ A plain baseline-ratchet is NOT sufficient: all four defects were on edges that already existed, so "fail when a NEW edge appears" would have passed them. The gate has to pair each eager→lazy / lazy→lazy edge with the door that awaits it. ⚠⚠ The suite can never cover this: the harness page and `index-sw.html` preset `FIZZYGUM_EAGER_ALL_PARTS`, so laziness is only real on `index.html`, which the suite never drives.
