...
- [ ] **FOUND WHILE LANDING P6, deliberately NOT fixed there (out of scope, real behavioural blast radius): `BackBufferMixin.isTransparentAt` can never answer true.** It reads `data.a is 0` off the `Color` that `getPixelColor` returns — but a `Color`'s alpha field is `_a` and there is no public `a`, so the test is `undefined is 0` ⇒ always false. Every back-buffered widget (`StringWdgt`, `CanvasWdgt`, `PaletteWdgt`) therefore reports itself OPAQUE at every point inside its bounds, and a click on a see-through pixel of a string or a canvas is never passed through to what is behind it. P6 added `Color.isFullyTransparent` (which is the correct spelling for this) for its own pick guard, so the repair is one line — but changing it flips hit-testing across the whole text layer, which wants its own before/after and its own recapture pass. ⚠ Check `@texture`'s early return and `boundsContainPoint` at the same time
...
- [ ] **open Q — geometry pins: REFRAMED 2026-08-17, and the deciding fact is a MEASUREMENT, not an argument.** `width`/`height` are write-only today as a HOLDING position (costs nothing — neither consumer exists yet — and is one line to reverse). The one-way law is a consequence of the cycle ORDER (`recalculateDataflow` runs before `recalculateLayouts`, so layout→dataflow has already missed its station). ⭐ But layout ALREADY notifies dependents by hand — `_reLayoutScrollbars` → `@hBar.updateSpecs` IS "layout settled, tell the bound thing" (gap G5) — so routing it through the drain generalises an existing direction rather than inventing one. The real cost is narrow: **a TRACKING PAIR shears by one frame under fast motion**, which is complaint ① exactly. Live candidates: **(c)** notify at end of layout, drain next frame (one frame of lag, no extra work) and **(d)** same-frame drain + a second bounded `recalculateLayouts` (no lag; the re-entrancy guard only forbids re-entry from INSIDE a pass). ⛔ CLOSED: (b) announce at the desired funnel (that funnel is settle-reachable, so it IS the violation) and (b′) announce-unless-flushing (silently wrong for engine-driven changes — a reader right *sometimes* is worse than none). ⇒ **§P8 is the experiment**: build its reverse channel against (c), drive a fast wheel-scroll, look at whether the bar shears, and record it. (e) readable-for-display-but-not-bindable is orthogonal and lands with P2, which must answer "which pins can be bound?" anyway.
- [ ] **P1 residue — a gate for "a pin's setter must exist".** P1 found `StringWdgt` advertising a `bang` pin implemented only on `SimpleTextWdgt`: the menu offered a target property whose dispatch (`consumer[action]?.call`) swallowed the miss, so it silently did nothing, indefinitely. Nothing catches this class — `check-unresolved-sends.js` cannot see a computed-name dispatch, and the new `pins` declarations are data. A `PinSpec` naming a setter/getter that does not resolve is a **sound negative** ⇒ hard gate, per `architecture/lint-and-static-checks.md` §3b. ⚠ **The design question that must be answered first:** `PatchNodeWdgt._inputPins` legitimately declares `setInput1..4`, which only its (three) subclasses implement — the base is abstract. So the rule is not "resolves on the declaring class" but "resolves on every CONCRETE class that inherits the pin", and the tree has no abstract/concrete marker today. Options: (a) add one (a class-level `isAbstract`, useful beyond this), (b) resolve DOWNWARD — a pin is OK if every leaf subclass implements it, (c) allowlist the abstract bases. Scoped deliberately OUT of P1: discovered there, not part of it, and worth doing properly.
