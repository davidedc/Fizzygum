...
- [ ] **VERIFY (pre-existing, surfaced by P5+P7): `PreferencesAndSettings` carries neither `keptByReferenceOnDeepCopy` nor `wellKnownKey`, but `Wallpaper` carries both.** A world-menu row has long held the prefs bag as its `target` ("touch screen settings" → `toggleInputMode`), so DUPLICATING that menu may clone the bag — and the copy's row would then toggle a dead clone, which is exactly the failure `Wallpaper`'s two markers exist to prevent. P5+P7 did not introduce this (the target reference predates it) but did add a second reference to the same object, and the row now READS the source rather than only clicking it. ⇒ Verify with a probe (duplicate the world menu, click the copy's input-mode row, check whether the REAL `WorldWdgt.preferencesAndSettings` changed) before deciding; if it clones, add both markers as `Wallpaper` has them. ⚠ Verify, don't assume — the bag is reached as a STATIC, which may take it off the duplication walk entirely.
