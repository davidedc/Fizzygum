...
- [x] **DONE 2026-08-25 — failure-image retention no longer leaves review blind spots, and the real mechanism was found.** (owner-filed 2026-08-25, from Plan 3's wave review): `AutomatorPlayer.FULL_FAILURE_IMAGE_BUDGET` caps how many full failure images a failing test retains (lowered 24 -> 8 during the visual wave when a shard crashed at the 4x pixel area of the 1920x880 world), so a test with more changed images than the budget shows committed references with NO corresponding "now" image in a pre-recapture review page -- the reviewer sees "broken image" and cannot eyeball the tail of the storyboard (hit live: `macroWindowsNestedCollapsingUncollapsing`, 13 images, tail 9-13 unreviewable). The budget is a pixel budget wearing a count's clothes; the fix should make review completeness structural rather than budget-dependent -- e.g. spill failure images to disk incrementally instead of holding them in memory, or scale the count budget by image byte-size, or have the review tooling fall back to a per-test full-storyboard dump when the budget truncated. Owner ruling at the wave: proceed with the gap, file this. UPDATE 2026-08-25: the same budget leaked into `--capture-ref` and wrote ZERO-BYTE reference PNGs (`dataHash0`) for images 9+ of every test with >8 screenshots -- fixed at the root (`AutomatorPlayer.failureImageBudget()` honours `window.FIZZYGUM_FAILURE_IMAGE_BUDGET`; the single-test runner lifts it), so capture completeness no longer depends on the budget. RESOLVED ROOT (2026-08-25, superseding the streaming proposal): the retention cost was never count or transfer per se -- `SWCanvasElement.toDataURL` uses SWCanvas's own JS `PngEncoder`, which emits an essentially UNCOMPRESSED ~36 MB data URL per 3840x1760 image (19-shot dpr-2 test ~= 684 MB of page strings -> the CDP protocol ceiling). Fix: `AutomatorPlayer.pngDataURLOf` blits the SWCanvas pixels onto ONE reused native scratch canvas and uses the native encoder -- 981 ms -> 31 ms, 36 MB -> 164 KB, `putImageData` lossless (raw-pixel dataHash matching is untouched by PNG-byte differences; old-encoder references still match, proven). The 19-shot dpr-2 test: >600 s failing -> 17.3 s green under FULL retention. Streaming is unnecessary.
- [ ] **Mass recapture is serialized behind single-threaded zopflipng** (coordinator-filed 2026-08-25, from Plan 3's wave): capturing 3,800 references at the 1920x880 world ran at ~0.5-0.8 tests/min (~7 h for the full wave) with ONE zopflipng process pinned at 98% CPU and machine load 2.6 -- the optimizer dominates at 4x pixel area and the capture is serialized behind it. The suite itself stays in envelope (~2 min parallel dpr1); this is recapture-only cost. Fix candidates: parallelize the zopflipng passes across cores; or capture with cheap PNG encoding and re-optimize asynchronously/in a follow-up pass; or lower optimizer effort for reference PNGs (they are hash-compared, not size-critical -- though repo size argues for keeping good compression somewhere in the loop). NOTE 2026-08-25: the IN-PAGE half of capture cost was separately fixed (SWCanvas JS PngEncoder -> native blit, see the resolved retention entry above); zopflipng remains the sole wall-time term for mass recapture.
...
- [ ] **remainder-menu alternative projections**: the chevron's overflow menu ships icon+label rows;
      a grid pop ("a vertical toolbar") or a label-only pop are available BY DERIVATION on the same
      `CommandSpec` rows, one line each — land when a consumer asks. Origin:
      `archive/command-panel-unification-plan.md` §2.2/OD2.
- [ ] **`PaintToolbarWdgt.queue: undefined` looks mis-homed**: the field is declared on the palette
      class, but its only reads/writes sit inside `TOOTHPASTE_TOOL_SOURCE` — injectable source text
      evaluated with `@` bound to the drawing overlay it is injected onto
...
- [ ] **inspector row-locator: scroll-further beyond 3 nudges**: `clickOnListItemFromTopInspector` aims inside the row∩pane slice and the L3 locator nudges up to 3× toward an off-window row; a row still unreachable after that throws a named error (the honest floor — it has never fired in a green run). If a legitimately deeper list ever trips it, extend the nudge loop rather than the aim. Origin: `archive/widget-input-vocabulary-plan.md` P3.
- [ ] **`catchesPointerAt` allowlist-entry hygiene**: the build prints `[call-separation] NOTE — allowlist entry no longer self-only-public: catchesPointerAt` (pre-existing, advisory); re-derive or re-reason the entry. Origin: noted throughout the input-vocabulary arc's builds.
