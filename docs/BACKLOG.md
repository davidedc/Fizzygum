...
- [ ] `plans/boot-cost-reduction-plan.md` §2.2c-post — a tool-promotion shortlist from slice 3, all currently gitignored prototypes in `Fizzygum-tests/.scratch/`: (a) `fg fingerprint <profile> [ref]` wrapping the sibling-worktree baseline recipe — the plan's own standard needs it every slice and it has a silent trap (symlinking `Fizzygum-tests` makes the pre-compile driver write into the REAL build tree while the build writes the baseline: void measurement, no error); (b) `fg hypopart <paths…>` — evaluate a hypothetical part before moving a file. ⚠ (b) currently COPY-PASTES `check-part-edges.js`'s guard classifier; promoting the copy would recreate the one-rule-in-two-places shape that produced four bugs in arc 4, so the classifier must be extracted into a module both require.
- [ ] `plans/boot-cost-reduction-plan.md` §2.2c-post — **the same palette has different contents depending on how it was opened.** `SlidesToolbarWdgt` filters its two map tools with `if WorldMapCreatorButtonWdgt?`. Reached DOCKED (`SimpleSlideApp.launch` awaits `maps`) it has them; reached FLOATING (`ToolbarsApp` → `SuperToolbarWdgt` → `SlidesToolbarCreatorButtonWdgt`, which awaits only `authoring`) it silently does not. Not a crash — a quietly reduced palette, which is the documented hazard of using an existence guard where an await is needed. Pre-dates slice 3 and is unchanged by it. Same question for `DashboardsToolbarWdgt`'s plot tools.
