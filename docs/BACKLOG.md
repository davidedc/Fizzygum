...
- [ ] `archive/settle-tier-followups-examination-plan.md` Topic 3: audit that every non-settling private fn is named *NoSettle — never started — NOTE: **NOT subsumed** — the `tiernaming` gate audits the apply-2x2 corners (a `__commit*` leaf that fired the seam or a react step in its own scope, an `_apply*Base` bypass twin that fired the seam), and `check-layering.js` [L] only forbids the suffix on a notification callback; nothing anywhere asserts the POSITIVE. Cost is one census over `src/**/*.coffee` cross-referencing each `_`/`__`-prefixed method against whether its body reaches `_settleLayoutsAfter` — the parse `check-layering.js` already performs, reported as a count. Count 0 ⇒ the item drops.
