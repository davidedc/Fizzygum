...
- [ ] `architecture/lint-and-static-checks.md` ("a gate must be able to tell BLINDNESS from CLEANLINESS") — **give the last three suite-wide audit gates a positive-coverage assertion.** DONE for `capstone` + `paint-readonly` (shape 1: "the flag is armed on THIS world") and `storage` (shape 2: "the audit RAN TO COMPLETION", plus a `-SKIPPED:<what was missing>` marker on the silent early return, INVALID/exit 2 on a shortfall). REMAINING: `tiernaming`, `settle`, `revisits` — each still passes on "zero findings", which measuring nothing satisfies perfectly, so a prelude that fails to install or a wrapper that stops matching after a rename reads GREEN. ⚠ These three have no audit flag to check (two had theirs deleted as dead), so they want shape 2: count what the WRAPPERS actually observed, per test. ⭐ Motivating measurements: capstone reported ✓ PASSED on 307 real careless pushes with its flag lost; the storage audit's `if (!sorter || !bin || !shelf) return` could no-op a whole run in silence. Expect at least one of the three to be already partly blind.
- [ ] `archive/world-reset-by-reconstruction-plan.md` §6 (OWNER DECISION, the arc's one deliberate carry-over) — **rename or retire the `RESETWORLD_INCOMPLETE` ratchet.** Since Arc C it asserts CONSTRUCTION DETERMINISM (every world a page builds must fingerprint identically to the first, against a page-scoped baseline), but both its method name `_auditWorldResetCompletenessNoSettle` and its token still describe the retired reset-era semantics — and the token is what an engineer greps. ⚠ Blast radius is the reason this is its own pass rather than an end-of-arc tail: `scripts/lib/gate-tokens.js`, all three headless runners, the fuzz parser regexes and its selftest must move in LOCKSTEP, or the token stops being recognised and the gate goes quietly blind. Suggested name: `WORLD_CONSTRUCTION_DRIFT`.
