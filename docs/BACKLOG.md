...
- [ ] `architecture/lint-and-static-checks.md` ("a gate must be able to tell BLINDNESS from CLEANLINESS") — **give the remaining suite-wide audit gates a positive-coverage assertion.** `capstone` and `paint-readonly` now have one (a per-test "armed on THIS world" line, required to equal the test count); `tiernaming`, `settle`, `storage` and `revisits` do NOT and can still go quietly blind — each passes on "zero findings", which measuring nothing satisfies perfectly, so a prelude that fails to install or a wrapper that stops matching after a rename reads as GREEN. ⭐ The measurement that motivates it: with its flag lost on reconstructed worlds, capstone reported ✓ PASSED on 307 real careless pushes. ⚠ Two of these no longer have a flag to lose (their dead flags were deleted), so their assertion must count what the WRAPPERS observed — e.g. tier-naming already counts wrapped corners/callbacks at install; the per-test equivalent is what is missing. Expect this to find at least one leg already partly blind.
