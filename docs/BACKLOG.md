...
- [ ] `Fizzygum-tests/scripts/serialization-file-roundtrip-headless.js`: `dropPixelParity` settles by rAF frame COUNT (`R.settle(8)`) — cadence-sensitive under parallel gauntlet load (one in-wave hash mismatch observed 2026-07-27; serial retry + standalone both green; fixture is a plain FrameWdgt, unrelated to the Appearance-delegation arc). Fix shape: settle on layout/paint convergence (or a generous event-drain condition), not frame count — DETERMINISM.md §3's rule applied to the rig itself.
