...
- **Build/test loop:** from the umbrella, `./fg build` · `./fg suite` · `./fg gauntlet` (or from `Fizzygum/`:
  ...
  (`buildSystem/check-*.js`) — the whole checking system is documented in **`docs/lint-and-static-checks.md`**
  (gate inventory, rules [A]–[R], markers, ratchet idioms, how to add a gate — this plan adds rules the way
  that doc's §8 prescribes).
- **The runtime invariant these rules protect** (depth: `docs/layout-system-architecture-assessment.md`):
...
- **Naming tiers** (authority: `docs/layering-naming-convention.md`): public `name` = user-meaningful API;
...
`docs/lint-and-static-checks.md` §4):
...
- Its forward effect/settle closure runs over **`@`-self calls only**. This is deliberately NOT the rejected
  [G] transitive closure (`docs/lint-and-static-checks.md` §7: backward name-reachability through `.`-member
...
Letters continue `check-layering.js`'s [A]–[R]. Follow `docs/lint-and-static-checks.md` §8 ("How-to") for
...
- **T0 — tooling (pure tooling, zero behaviour change).**
  ...
  4. Document: new rows in `docs/lint-and-static-checks.md` §3 gate table + §4 rules; note the census tool
...
- **T2 — close the two demonstrated evasions + delete vestigial hooks.** (a) StretchableEditable family:
  ...
  `docs/all-constructors-settle-plan.md` before proceeding.
...
- **Backward transitive closure for [G]** — previously prototyped and REJECTED
  (`docs/lint-and-static-checks.md` §7, ~500–710 false hits). Unchanged. The census's *forward, @-self-only*
...
- **Serialization / dependency discovery are rename-safe** (`docs/layering-naming-convention.md` §6: the
...
- `docs/lint-and-static-checks.md` — the gate system this plan extends (rules [A]–[R], markers, ratchets, how-to).
- `docs/layering-naming-convention.md` — the tier scheme (`_`/`__`/NoSettle) and why names are load-bearing.
- `docs/layout-system-architecture-assessment.md` — the one-flush invariant (the *why* behind [S]/[T]).
- `docs/all-constructors-settle-plan.md` — the ctor pattern T2(b) folds ColorPickerWdgt into.
- `docs/lint-ratchet-static-checks-plan.md` — the rejected backward-transitive record (§8 boundary here).
