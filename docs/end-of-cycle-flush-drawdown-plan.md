...
revealed two public methods that should have self-settled and didn't. This plan repeats that, contributor by
contributor, down the inventory.
...
## 7. The current inventory — starting targets (after the `Widget.destroy` fix: 564 records)

By-action (interaction-frame records; from the post-destroy-fix audit). Re-run §4 to refresh before starting.

| action / origin | recs | first-pass classification (verify with the data) |
|---|--:|---|
| **(untagged) hover / pointer-dispatch** (`ActivePointerWdgt` mouseEnter/mouseLeave diff, `mouseOverList.forEach`) | 230 | **LEAVE** — continuous, every pointer move. Confirm it's pure hover. The single biggest residual; likely correct. |
| **`TextWdgt.reLayoutAndRefreshContainerIfContainedText`** (contained-text edit re-fit) | 124 | **PRIME CONVERT CANDIDATE** — the contained-text-edit seam. Mirror of the `sizeToTextAndDisableFitting` fix: a text edit should self-settle its container. The biggest convertible chunk. |
...
| `Widget.destroy` (residual, NON-freefloating) | 16 | **LEAVE** — genuine teardown of laid-out children; correctly dirties the parent. |
...
4. Confirm the **LEAVE** set (hover 230, drag ~75, residual destroy 16, paint 14, setMaxDim 4, softwrap 3) is
