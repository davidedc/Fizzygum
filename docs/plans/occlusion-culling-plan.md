...
`docs/archive/interactive-render-perf-A-C-plan.md` (which make the painting that DOES happen cheaper;
...
- **Landing**: Fizzygum repo only; phases land as one arc. Present per-phase summary, gate results
  ...
  After approval: record the outcome in `docs/plans/runtime-performance-optimization-plan.md` §8 ledger
  + memory. This plan composes with (does not depend on)
  `docs/archive/interactive-render-perf-A-C-plan.md`: that one makes the painting that DOES happen
