...
- Plan 3 visual wave — **IN EXECUTION 2026-08-24** (`single-geometry-visual-wave-plan.md`; its
  STATUS box is the log: P0–P1 done+pushed, P2 probe in owner iteration).
...
| T2 | **Overflow chevron** — a docked strip showing as many items as fit + an overflow button popping the rest as a menu (macOS/iPad toolbar convention; T1 from the other side). ⚠ Live motivation since P5: the grip bar eats strip length, so Docs Maker's last tool now sits behind the strip's scroll. | session | RE-ADDRESSED 2026-08-24 (owner ruling at Plan 3 P0): **ABSORBED into Plan 3** — with `toolRows 1` ruled, the chevron is part of the single-geometry toolbar answer, not an optional refinement; its pixels ride THE wave (+½ session-day). | open |
| T3 | **Resize handle: small glyph, large hit zone** (or edge-grab resizing). RULED 2026-08-24 (owner, Plan 3 P0, from the three shapes weighed in the plan's §2.5): **(a) big glyph — glyph = box, a 44 px drawn grip**; what you can hit is what you see. No `handleGlyphSize` preference is introduced; edge-grab was not wanted and gets no tail row. | session | Plan 3 P3 (lands with the dials); the P2 probe page confirms it by eye | open |
