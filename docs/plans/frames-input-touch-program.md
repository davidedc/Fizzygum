...
| T2 | **Overflow chevron** — a docked strip showing as many items as fit + an overflow button popping the rest as a menu (macOS/iPad toolbar convention; T1 from the other side). ⚠ Live motivation since P5: the grip bar eats strip length, so Docs Maker's last tool now sits behind the strip's scroll. | session | BACKLOG, after T1 | open |
