    ...
    if (this.children.length) {
      ...
      if (this.world.caret) {
        if (morph !== this.world.caret.target) {
          console.log("processMouseDown morph != caret target");
          this.world.stopEditing();
        }
      }
      if (!morph.mouseMove) {
...
CaretMorph = (function(_super) {
  ...
  function CaretMorph(target) {
    ...
    this.target = target;
    console.log("constructed new CaretMorph");
  ...
  CaretMorph.prototype.gotoSlot = function(slot) {
    ...
    pos = this.target.slotCoordinates(slot);
...
TextMorph = (function(_super) {
  ...
  TextMorph.prototype.slotCoordinates = function(slot) {
    var colRow, context, shadowHeight, x, xOffset, y, yOffset;

    colRow = this.slotRowAndColumn(slot);
    context = this.image.getContext("2d");
    shadowHeight = Math.abs(this.shadowOffset.y);
    yOffset = colRow.y * (fontHeight(this.fontSize) + shadowHeight);
    xOffset = context.measureText(this.lines[colRow.y].substring(0, colRow.x)).width;
    x = this.left() + xOffset;
    y = this.top() + yOffset;
    return new Point(x, y);
  };
